import { a as __exportAll, i as __commonJSMin, o as __toESM } from "../index.js";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { Fragment as Fragment$1, jsx, jsxs } from "react/jsx-runtime";
import minpath from "node:path";
import minproc from "node:process";
import { fileURLToPath as urlToPath } from "node:url";
//#region lib/orchestrator-client.ts
function requestId() {
	if (typeof crypto !== "undefined" && "randomUUID" in crypto) return `req_${crypto.randomUUID().replaceAll("-", "").slice(0, 12)}`;
...
function buildAgentRequest(input) {
	return {
		...
		profile_md_version: (/* @__PURE__ */ new Date()).toISOString().slice(0, 10),
...
function isAgentResponse(value) {
	if (!value || typeof value !== "object") return false;
	const candidate = value;
	return candidate.api_version === "v1" && typeof candidate.request_id === "string" && typeof candidate.status === "string" && Array.isArray(candidate.agent_trace);
}
async function dispatchToCentralOrchestrator(request) {
	...
	try {
		...
		if (!response.ok) {
			const message = data && typeof data === "object" && "error" in data ? String(data.error) : `中央调度器返回 HTTP ${response.status}`;
			throw new Error(message);
		}
		if (!isAgentResponse(data)) throw new Error("中央调度器返回的数据不符合 v1 契约");
		if (data.request_id !== request.request_id) throw new Error("返回的 request_id 与当前请求不一致");
		return data;
	} catch (error) {
		if (error instanceof DOMException && error.name === "AbortError") throw new Error("中央调度器响应超时，请检查多 Agent 服务");
		throw error;
	} finally {
...
//#endregion
//#region lib/profile-client.ts
async function checkBackendHealth() {
	const response = await fetch("/api/agent/health", { cache: "no-store" });
	const data = await response.json();
	if (!response.ok) throw new Error(data.error || "无法连接多 Agent 中央调度器");
	return data;
}
async function profileRequest(userId, init) {
	const response = await fetch(`/api/profile/${encodeURIComponent(userId)}`, init);
	const data = await response.json();
	if (!response.ok) throw new Error(data.error || `Memory 接口返回 HTTP ${response.status}`);
	return data;
}
function saveBackendProfile(userId, profile, scores) {
...
function mergeBackendProfile(data, currentProfile, currentScores) {
	const remoteProfile = data.profile || {};
	const level = remoteProfile.level;
	const profile = {
		...currentProfile,
		...typeof remoteProfile.background === "string" ? { background: remoteProfile.background } : {},
		...level === "beginner" || level === "intermediate" || level === "advanced" ? { level } : {},
		...typeof remoteProfile.preference === "string" ? { preference: remoteProfile.preference } : {}
	};
...
//#endregion
//#region lib/workspace-state-client.ts
async function stateRequest(userId, init) {
...
function loadBackendWorkspaceState(userId) {
	return stateRequest(userId);
}
function saveBackendWorkspaceState(userId, payload) {
	const send = (body) => stateRequest(userId, {
		method: "PUT",
		headers: { "Content-Type": "application/json" },
		body: JSON.stringify(body)
	});
	return send(payload).catch((error) => {
		if (error.status !== 422) throw error;
		const legacyFrontendState = Object.fromEntries(Object.entries(payload.frontend_state).filter(([key]) => key !== "capability_assessment" && key !== "quiz_sessions" && key !== "active_quiz_session_id" && key !== "learning_progress" && key !== "lecture_sessions" && key !== "active_lecture_session_id"));
		return send({
			...payload,
...
*/
var CNC_JOB_CAPABILITY_WEIGHTS = {
	safety: 15,
	foundations: 12,
	process_planning: 14,
	programming: 16,
...
var DIMENSION_SOURCE_RELIABILITY = {
	declared: 1,
	keyword: .8,
	fallback: .5
};
//#endregion
//#region lib/capability-assessment.ts
var ASSESSMENT_MODEL_VERSION = "cnc-capability-v2";
/**
...
function normalizeKnowledgePointId(value) {
	return String(value ?? "").normalize("NFKC").toLowerCase().replace(/[^\p{L}\p{N}]+/gu, "").slice(0, 180);
}
function itemFingerprint(question) {
	return normalizeKnowledgePointId(question.stem).slice(0, 240);
}
function asDifficulty(value) {
	const text = normalizedText(value);
	if (text.includes("hard") || text.includes("进阶") || text.includes("高级")) return "hard";
	if (text.includes("medium") || text.includes("中等") || text.includes("中级")) return "medium";
	return "easy";
}
function declaredDimension(value) {
	return {
		safety: "safety",
		安全: "safety",
		theory: "foundations",
...
function classifyCapabilityDimensionWithSource(input) {
	...
	for (const dimension of CAPABILITY_DIMENSIONS) {
		const hits = dimension.keywords.reduce((total, keyword) => total + (text.includes(normalizedText(keyword)) ? 1 : 0), 0);
		if (hits > bestHits) {
			best = dimension.id;
			bestHits = hits;
		}
...
function responseGrounding$1(response) {
	...
	return {
		...
		ragChunkIds: uniqueStrings$2([...evidence.map((item) => item.chunk_id), ...citations.map((item) => item.chunk_id)])
	};
}
function createQuizEvidence(attempt) {
	const grounding = responseGrounding$1(attempt.response);
	return attempt.questions.map((item, index) => {
		...
		const knowledgePoint = question.knowledge_point?.trim() || question.stem.trim().slice(0, 120);
		const questionSpecificSources = uniqueStrings$2(question.source_refs ?? []);
		const questionSpecificChunks = uniqueStrings$2(question.rag_chunk_ids ?? []);
		const questionGrounded = question.source_grounding_scope === "question" || question.source_grounding_scope === void 0 && (questionSpecificSources.length > 0 || questionSpecificChunks.length > 0);
		const possible = Number.isFinite(Number(item.possible)) && Number(item.possible) > 0 ? Number(item.possible) : 1;
		const earned = Number.isFinite(Number(item.earned)) ? Math.max(0, Math.min(possible, Number(item.earned))) : item.selectedAnswer === item.correctAnswer ? possible : 0;
		const correct = typeof item.isCorrect === "boolean" ? item.isCorrect : earned >= possible * .6;
		const subjective = question.question_type === "cloze" || question.question_type === "short_answer";
		const graderConfidence = typeof item.graderConfidence === "number" ? Math.max(0, Math.min(1, item.graderConfidence)) : null;
		return {
			...
			sourceRefs: uniqueStrings$2([...question.source_refs ?? [], ...grounding.sourceRefs]),
			ragChunkIds: uniqueStrings$2([...question.rag_chunk_ids ?? [], ...grounding.ragChunkIds]),
			...
			graderConfidence,
			questionGrounded,
			reviewStatus: subjective && (graderConfidence === null || graderConfidence < .6) ? "pending_review" : "auto_verified",
			chapterId: attempt.chapterId,
			lectureId: attempt.lectureId,
			objectiveIds: uniqueStrings$2(attempt.objectiveIds ?? []),
			criticalSafetyError: item.criticalSafetyError === true
		};
	});
}
function mergeCapabilityEvidence(current, incoming) {
	const byId = /* @__PURE__ */ new Map();
	for (const item of [...incoming, ...current]) if (!byId.has(item.id)) byId.set(item.id, item);
	return [...byId.values()].sort((a, b) => new Date(b.occurredAt).getTime() - new Date(a.occurredAt).getTime()).slice(0, 2e3);
}
function normalizeCapabilityEvidence(value) {
	...
	return value.flatMap((candidate) => {
		...
		if (!dimension || typeof item.id !== "string" || typeof item.attemptId !== "string" || typeof item.topic !== "string" || typeof item.knowledgePoint !== "string" || typeof item.correct !== "boolean" || typeof item.occurredAt !== "string") return [];
		const earned = Number(item.earned);
		const possible = Number(item.possible);
		if (!Number.isFinite(earned) || !Number.isFinite(possible) || possible <= 0) return [];
		return [{
			id: item.id,
			attemptId: item.attemptId,
			attemptNumber: Math.max(1, Math.trunc(Number(item.attemptNumber) || 1)),
			sourceType: item.sourceType === "practice" || item.sourceType === "external_assessment" ? item.sourceType : "quiz",
			dimension,
			dimensionSource: item.dimensionSource === "declared" || item.dimensionSource === "keyword" || item.dimensionSource === "fallback" ? item.dimensionSource : "fallback",
			topic: item.topic,
			knowledgePoint: item.knowledgePoint,
			knowledgePointId: typeof item.knowledgePointId === "string" && item.knowledgePointId ? item.knowledgePointId : normalizeKnowledgePointId(item.knowledgePoint),
			itemRevision: typeof item.itemRevision === "string" && item.itemRevision ? item.itemRevision : normalizeKnowledgePointId(item.knowledgePoint),
			correct: item.correct,
			earned,
			possible,
			difficulty: asDifficulty(item.difficulty),
			occurredAt: item.occurredAt,
			sourceRefs: Array.isArray(item.sourceRefs) ? uniqueStrings$2(item.sourceRefs.filter((source) => typeof source === "string")) : [],
			ragChunkIds: Array.isArray(item.ragChunkIds) ? uniqueStrings$2(item.ragChunkIds.filter((chunk) => typeof chunk === "string")) : [],
			questionType: typeof item.questionType === "string" ? item.questionType : void 0,
			gradingMethod: typeof item.gradingMethod === "string" ? item.gradingMethod : void 0,
			rubricVersion: typeof item.rubricVersion === "string" ? item.rubricVersion : void 0,
			semanticScore: Number.isFinite(Number(item.semanticScore)) ? Number(item.semanticScore) : null,
			keyPointScore: Number.isFinite(Number(item.keyPointScore)) ? Number(item.keyPointScore) : null,
			graderConfidence: Number.isFinite(Number(item.graderConfidence)) ? Math.max(0, Math.min(1, Number(item.graderConfidence))) : null,
			questionGrounded: item.questionGrounded === true,
			reviewStatus: item.reviewStatus === "reviewed" || item.reviewStatus === "rejected" ? item.reviewStatus : item.sourceType === "practice" || item.sourceType === "external_assessment" ? "pending_review" : "auto_verified",
			reviewedBy: typeof item.reviewedBy === "string" ? item.reviewedBy : void 0,
			lectureId: typeof item.lectureId === "string" ? item.lectureId : void 0,
			chapterId: typeof item.chapterId === "string" ? item.chapterId : void 0,
			objectiveIds: Array.isArray(item.objectiveIds) ? uniqueStrings$2(item.objectiveIds.filter((objective) => typeof objective === "string")) : [],
			criticalSafetyError: item.criticalSafetyError === true
		}];
	});
}
function validEvidence(item) {
	return CAPABILITY_DIMENSIONS.some((dimension) => dimension.id === item.dimension) && Number.isFinite(item.earned) && Number.isFinite(item.possible) && item.possible > 0 && item.earned >= 0 && item.earned <= item.possible;
}
function recencyWeight(occurredAt, now) {
	const timestamp = new Date(occurredAt).getTime();
	if (!Number.isFinite(timestamp)) return 1;
	const ageDays = Math.max(0, (now.getTime() - timestamp) / 864e5);
	return Math.pow(.5, ageDays / 180);
}
function masteryLabel(score, status) {
...
function evidenceIsReviewed(item) {
...
*/
function effectiveCapabilityEvidence(evidence) {
	const sorted = [...evidence].filter((item) => validEvidence(item) && evidenceIsReviewed(item)).sort((a, b) => new Date(a.occurredAt).getTime() - new Date(b.occurredAt).getTime());
	const seenItems = /* @__PURE__ */ new Set();
	const seenAttemptKnowledge = /* @__PURE__ */ new Set();
	return sorted.filter((item) => {
		const fingerprint = `${item.dimension}|${item.itemRevision || item.id}`;
		const attemptKnowledge = `${item.attemptId}|${item.dimension}|${item.knowledgePointId || normalizeKnowledgePointId(item.knowledgePoint)}`;
		if (seenItems.has(fingerprint) || seenAttemptKnowledge.has(attemptKnowledge)) return false;
		seenItems.add(fingerprint);
		seenAttemptKnowledge.add(attemptKnowledge);
		return true;
	});
}
function capabilityEvidenceWeight(item, now) {
	const graderReliability = typeof item.graderConfidence === "number" ? .35 + Math.max(0, Math.min(1, item.graderConfidence)) * .65 : 1;
	return DIFFICULTY_WEIGHT[item.difficulty] * recencyWeight(item.occurredAt, now) * SOURCE_RELIABILITY[item.sourceType] * DIMENSION_SOURCE_RELIABILITY[item.dimensionSource || "fallback"] * graderReliability;
}
function calculateScoreSlice(items, now) {
	...
	const weightedAccuracy = possibleWeight ? Math.max(0, Math.min(1, earnedWeight / possibleWeight)) : null;
	const estimated = weightedAccuracy === null ? null : (earnedWeight + CAPABILITY_RATING_POLICY.priorMean * CAPABILITY_RATING_POLICY.priorStrength) / (possibleWeight + CAPABILITY_RATING_POLICY.priorStrength);
	const knowledgePointCount = new Set(items.map((item) => item.knowledgePointId).filter(Boolean)).size;
	...
	const volumeConfidence = Math.min(1, items.length / CAPABILITY_RATING_POLICY.evidenceForFullConfidence);
	const diversityConfidence = Math.min(1, knowledgePointCount / CAPABILITY_RATING_POLICY.knowledgePointsForFullConfidence);
	const attemptConfidence = Math.min(1, independentAttemptCount / CAPABILITY_RATING_POLICY.attemptsForFullConfidence);
	const groundingConfidence = items.length ? groundedCount / items.length : 0;
	const confidence = Math.min(1, volumeConfidence * .45 + diversityConfidence * .25 + attemptConfidence * .2 + groundingConfidence * .1);
	const rated = items.length >= CAPABILITY_RATING_POLICY.minimumEffectiveEvidence && knowledgePointCount >= CAPABILITY_RATING_POLICY.minimumKnowledgePoints && independentAttemptCount >= CAPABILITY_RATING_POLICY.minimumIndependentAttempts && confidence >= CAPABILITY_RATING_POLICY.minimumConfidence;
	return {
		score: estimated === null ? null : Math.round(estimated * 100),
		observedScore: weightedAccuracy === null ? null : Math.round(weightedAccuracy * 100),
		weightedAccuracy,
...
function confidenceLabel(evidenceCount, confidence) {
	if (!evidenceCount) return "待评估";
	if (confidence >= .75) return "高";
	if (confidence >= .4) return "中";
	return "低";
}
function calculateCapabilityAssessment(evidence, now = /* @__PURE__ */ new Date()) {
	...
	const entries = CAPABILITY_DIMENSIONS.map((definition) => {
		...
		const knowledgeItems = items.filter((item) => item.sourceType === "quiz" || item.sourceType === "external_assessment");
		const practiceItems = items.filter((item) => item.sourceType === "practice" || item.sourceType === "external_assessment");
		...
		const result = {
			...
			practiceEvidenceCount: practiceItems.filter((item) => item.sourceType === "practice").length,
			externalEvidenceCount: practiceItems.filter((item) => item.sourceType === "external_assessment").length,
			sourceCount: new Set(items.flatMap((item) => item.sourceRefs)).size,
			lastUpdated: rawItems.length ? [...rawItems].sort((a, b) => new Date(b.occurredAt).getTime() - new Date(a.occurredAt).getTime())[0].occurredAt : null
		};
		return [definition.id, result];
	});
	const dimensions = Object.fromEntries(entries);
	return {
		modelVersion: ASSESSMENT_MODEL_VERSION,
		dimensions,
		evidenceCount: accepted.length,
		effectiveEvidenceCount: effective.length,
		assessedDimensionCount: Object.values(dimensions).filter((item) => item.evidenceCount > 0).length,
		ratedDimensionCount: Object.values(dimensions).filter((item) => item.ratingReady).length
	};
}
function averageRated(assessment, ids) {
	const scores = ids.map((id) => assessment.dimensions[id]).filter((item) => item.ratingReady).map((item) => item.score).filter((score) => score !== null);
	return scores.length ? scores.reduce((total, score) => total + score, 0) / scores.length : 0;
}
function assessmentToScoreMap(assessment, _level) {
	const scores = Object.fromEntries(CAPABILITY_DIMENSIONS.map((dimension) => [dimension.id, assessment.dimensions[dimension.id].ratingReady ? assessment.dimensions[dimension.id].score ?? 0 : 0]));
	let weightedTotal = 0;
	...
	for (const dimension of CAPABILITY_DIMENSIONS) {
		...
		if (result.ratingReady) weightedTotal += (result.score ?? 0) * (weight / 100);
	...
	scores.theory = Math.round(averageRated(assessment, [
		"foundations",
		"process_planning",
		"quality_control"
	]));
	scores.safety = Math.round(assessment.dimensions.safety.ratingReady ? assessment.dimensions.safety.score ?? 0 : 0);
	scores.operation = Math.round(averageRated(assessment, [
		"machining_operation",
		"quality_control",
		"maintenance"
	]));
	scores.programming = Math.round(assessment.dimensions.programming.ratingReady ? assessment.dimensions.programming.score ?? 0 : 0);
	scores.overall = Math.round(weightedTotal);
	scores.provisional_overall = Math.round(provisionalWeightedTotal);
	scores.assessment_confidence = Math.round(Object.values(assessment.dimensions).reduce((total, item) => total + item.confidence, 0) / CAPABILITY_DIMENSIONS.length * 100);
	return scores;
}
function capabilityResultList(assessment, level) {
	return CAPABILITY_DIMENSIONS.filter((dimension) => dimension.id !== "advanced_manufacturing" || level === "advanced" || assessment.dimensions.advanced_manufacturing.evidenceCount > 0).map((dimension) => assessment.dimensions[dimension.id]);
}
//#endregion
//#region lib/learning-recommendations.ts
var CNC_TRACKS = [
	{
		id: "safety",
		label: "数控机床安全操作",
		...
		chatTask: "讲清数控机床开机前检查、个人防护、急停操作和异常情况处置",
		quizTopic: "数控机床安全操作",
	...
	},
	{
		...
		chatTask: "说明数控车铣加工从工件装夹、刀具安装、对刀到首件试切的标准流程",
	...
	},
	{
		...
		chatTask: "解释数控加工坐标系、刀具补偿、切削用量与加工精度之间的关系",
		quizTopic: "数控加工基础理论",
	...
	},
	{
		...
		chatTask: "结合简单加工案例解释常用 G/M 代码、程序结构和程序校验方法",
	...
	},
	{
		...
		chatTask: "对比三轴与多轴加工，说明旋转轴、联动加工、后处理和防碰撞要点",
		quizTopic: "多轴数控加工基础",
	...
	},
	{
		...
		chatTask: "梳理数控车铣加工职业技能等级考核的知识要求、技能要求和复习重点",
...
function profileText(profile) {
	return Object.values(profile).flatMap((value) => Array.isArray(value) ? value : [value]).map(normalize$1).filter(Boolean).join(" ");
}
function memoryText(events) {
	return events.slice(0, 12).flatMap((event) => [event.title, event.detail]).map(normalize$1).filter(Boolean).join(" ");
}
function ragText(ragPackage) {
	if (!ragPackage) return "";
	return [
		ragPackage.query,
		ragPackage.answer,
		...(ragPackage.evidence ?? []).flatMap((item) => [item.source_file ?? item.source_doc, item.text])
	].map(normalize$1).filter(Boolean).join(" ");
}
function keywordHits(text, keywords) {
	if (!text) return 0;
	return keywords.reduce((total, keyword) => {
		const token = normalize$1(keyword);
...
function scoreLabel(id) {
	return CAPABILITY_DIMENSIONS.find((dimension) => dimension.id === id)?.shortLabel || id;
}
function learningStyle(profile) {
...
function displaySourceName(source) {
	...
	const name = source.split(/[\\/]/).pop()?.replace(/\.(pdf|docx?|xlsx?|pptx?|txt|md)$/i, "").replace(/[_-]+/g, " ").trim();
	if (!name) return null;
	return name.slice(0, 48);
}
function ragSourceName(ragPackage) {
	const evidence = [...ragPackage?.evidence ?? []].sort((a, b) => Number(b.score ?? 0) - Number(a.score ?? 0));
	return displaySourceName(evidence[0]?.source_file ?? evidence[0]?.source_doc);
}
/**
...
*/
function buildLearningRecommendations({ profile, scores, memoryEvents = [], ragPackage = null }) {
	...
	const ranked = CNC_TRACKS.map((track, index) => {
		...
		return {
			track,
			rank: track.defaultPriority + gap / 18 + memoryHits * 3.5 + ragHits * 4.5 - index * .001,
			memoryHits,
			ragHits
		};
	...
	const origin = anyRagSignal ? "rag" : anyMemoryTopic ? "memory" : "domain-default";
	const weakest = ranked[0].track;
	const weakestScore = Math.round(clampScore(scores[weakest.scoreDimension]));
	const contextLabel = anyRagSignal ? `已结合 Memory 与知识资料，当前优先：${weakest.label}` : `已根据 Memory 调整，当前优先：${weakest.label}（${weakestScore} 分）`;
	const chatPrompts = ranked.slice(0, 4).map(({ track }) => `${style}${track.chatTask}`);
	const quizOptions = ranked.slice(0, 3).map(({ track, memoryHits, ragHits }) => ({
		id: track.id,
		topic: track.quizTopic,
		focus: `${track.quizFocus}；${difficultyHint(profile.level)}`,
		reason: ragHits > 0 ? "匹配当前知识资料" : memoryHits > 0 ? "匹配近期 Memory" : `${scoreLabel(track.scoreDimension)} ${Math.round(clampScore(scores[track.scoreDimension]))} 分`
	}));
	if (sourceName) {
...
//#endregion
//#region components/UserIdentity.tsx
var AVATAR_PALETTES = [
	["#fff0d9", "#ffc98d"],
	["#f1ecff", "#bca9ff"],
	["#e4f7ff", "#92d8f5"],
...
].map(([id, emoji, name], index) => ({
	id,
	emoji,
	name,
	colors: AVATAR_PALETTES[index % AVATAR_PALETTES.length]
}));
var DEFAULT_USER_IDENTITY = {
	nickname: "学习者",
	avatarId: "fox"
};
function isProvidedAvatar(avatarId) {
	return AVATAR_CHOICES.some((avatar) => avatar.id === avatarId);
}
function avatarById(avatarId) {
	return AVATAR_CHOICES.find((avatar) => avatar.id === avatarId) || AVATAR_CHOICES.find((avatar) => avatar.id === DEFAULT_USER_IDENTITY.avatarId) || AVATAR_CHOICES[0];
}
function UserAvatar({ avatarId, className = "" }) {
...
function UserProfileControl({ identity, onChange }) {
	...
	useEffect(() => {
		...
		function closeOnOutsideClick(event) {
			if (event.target instanceof Node && !controlRef.current?.contains(event.target)) setMenuOpen(false);
		}
		...
		children: [
			...
			/* @__PURE__ */ jsxs("button", {
				...
				children: [
					...
					/* @__PURE__ */ jsxs("span", {
						className: "user-profile-copy",
						children: [/* @__PURE__ */ jsx("strong", { children: identity.nickname }), /* @__PURE__ */ jsx("small", { children: "本地学习者" })]
					}),
					/* @__PURE__ */ jsx("span", {
			...
			dialog === "avatar" && /* @__PURE__ */ jsx("div", {
				...
				children: /* @__PURE__ */ jsxs("section", {
					...
					children: [/* @__PURE__ */ jsxs("header", {
						...
						children: [/* @__PURE__ */ jsxs("div", { children: [
							...
							/* @__PURE__ */ jsx("p", { children: "从 50 个可爱动物伙伴中选择一个。" })
						] }), /* @__PURE__ */ jsx("button", {
							type: "button",
						...
						children: AVATAR_CHOICES.map((avatar) => /* @__PURE__ */ jsxs("button", {
							type: "button",
							className: `avatar-choice ${avatar.id === identity.avatarId ? "selected" : ""}`,
							...
							}), /* @__PURE__ */ jsx("span", { children: avatar.name })]
						}, avatar.id))
					})]
				})
			}),
			dialog === "nickname" && /* @__PURE__ */ jsx("div", {
				className: "user-settings-backdrop",
				onMouseDown: (event) => {
					if (event.target === event.currentTarget) setDialog(null);
				},
				children: /* @__PURE__ */ jsxs("section", {
					...
					children: [/* @__PURE__ */ jsxs("header", {
						...
						children: [/* @__PURE__ */ jsxs("div", { children: [
							...
							/* @__PURE__ */ jsx("p", { children: "昵称会显示在左下角和你的聊天消息旁。" })
						] }), /* @__PURE__ */ jsx("button", {
							type: "button",
							className: "settings-close-button",
							"aria-label": "关闭昵称编辑",
						...
						children: [
							...
							/* @__PURE__ */ jsxs("div", {
								className: "nickname-form-meta",
								children: [/* @__PURE__ */ jsx("span", {
									className: nicknameError ? "nickname-error" : "",
									children: nicknameError || "最多 20 个字符"
								}), /* @__PURE__ */ jsxs("span", { children: [nicknameDraft.length, "/20"] })]
							}),
							/* @__PURE__ */ jsxs("div", {
...
function createElementProps(state, node) {
	...
	/** @type {string} */
	let prop;
	for (prop in node.properties) if (prop !== "children" && own$4.call(node.properties, prop)) {
...
function MarkdownContent({ content, baseUrl }) {
		...
		children: /* @__PURE__ */ jsx(Markdown, {
			...
			components: {
				a: ({ children, href, ...props }) => {
					...
					if (resolvedHref && isVideoUrl(resolvedHref)) return /* @__PURE__ */ jsx("video", {
					...
					return /* @__PURE__ */ jsx("a", {
						...props,
						href: resolvedHref || rawHref || void 0,
						target: "_blank",
				...
				img: ({ alt, src, ...props }) => {
					...
					if (resolvedSrc && isVideoUrl(resolvedSrc)) return /* @__PURE__ */ jsx("video", {
						className: "markdown-video",
						controls: true,
						preload: "metadata",
						src: resolvedSrc,
					...
					return /* @__PURE__ */ jsx("img", {
						...props,
						alt: alt || "",
						src: resolvedSrc || rawSrc || void 0,
						loading: "lazy"
					});
				},
				table: ({ children, ...props }) => /* @__PURE__ */ jsx("div", {
			...
			children: normalizeSectionHeadings(content)
		})
	});
}
//#endregion
//#region lib/quiz-session.ts
var QUIZ_SESSION_MODEL_VERSION = "zlink-quiz-session-v3";
function asRecord$2(value) {
	return value && typeof value === "object" ? value : null;
}
function cleanString$1(value, max = 2e4) {
	return typeof value === "string" ? value.trim().slice(0, max) : "";
}
function uniqueStrings$1(values, max = 100) {
	return [...new Set(values.filter((value) => typeof value === "string").map((value) => value.trim()).filter(Boolean))].slice(0, max);
}
function difficulty(value) {
	const normalized = cleanString$1(value, 30).toLowerCase();
	if (normalized === "hard" || normalized.includes("进阶")) return "hard";
	if (normalized === "medium" || normalized.includes("中等")) return "medium";
	return "easy";
}
function validDate$1(value, fallback) {
	const text = cleanString$1(value, 80);
	return text && Number.isFinite(new Date(text).getTime()) ? text : fallback;
}
function quizAnswerKey(question) {
	if (quizQuestionType(question) === "true_false") return normalizeTrueFalseAnswer(question);
	const raw = cleanString$1(question.answer, 500).toUpperCase();
	if (/^[A-Z]$/.test(raw)) return raw;
	const optionIndex = question.options.findIndex((option) => option === question.answer);
	return optionIndex >= 0 ? String.fromCharCode(65 + optionIndex) : raw.slice(0, 1);
}
var TRUE_ANSWER_ALIASES = new Set([
	"a",
	"正确",
	"对",
	"是",
...
var FALSE_ANSWER_ALIASES = new Set([
	"b",
	"错误",
	"错",
	"否",
...
function normalizedBooleanToken(value) {
	return cleanString$1(value, 500).toLowerCase().replace(/[\s。．，,：:；;！!？?（）()【】\[\]]/g, "");
}
function normalizeTrueFalseAnswer(question) {
	...
	const letter = cleanString$1(question.answer, 20).trim().toUpperCase();
	if (/^[A-Z]$/.test(letter)) {
	...
	const matchingToken = normalizedBooleanToken(question.options.find((option) => normalizedBooleanToken(option) === raw));
...
function quizQuestionType(question) {
	const value = cleanString$1(question.question_type, 40).toLowerCase();
...
function quizQuestionPoints(question) {
	...
	const difficulty = cleanString$1(question.difficulty, 50).toLowerCase();
	return difficulty.includes("hard") || difficulty.includes("进阶") ? 12 : difficulty.includes("medium") || difficulty.includes("中等") ? 10 : 7;
}
function isSubjectiveQuizQuestion(question) {
	const type = quizQuestionType(question);
	return type === "cloze" || type === "short_answer";
}
function conciseExplanation(text) {
	const normalized = cleanString$1(text, 4e4).replace(/\s+/g, " ");
	if (!normalized) return "暂无解析。";
	const summary = (normalized.match(/.*?[。！？!?](?=\s|$|[^。！？!?])/g) ?? [])[0]?.trim() || normalized;
	return summary.length > 140 ? `${summary.slice(0, 137)}…` : summary;
}
function answerForDisplay(question) {
	if (quizQuestionType(question) === "true_false") return normalizeTrueFalseAnswer(question) === "A" ? "正确" : "错误";
	...
	return cleanString$1(question.reference_answer || question.answer, 2e4);
}
/**
...
*/
function quizExplanationText(question, mode) {
	const legacy = cleanString$1(question.explanation, 4e4);
	const explicitConcise = cleanString$1(question.concise_explanation, 1e4);
	const explicitDetailed = cleanString$1(question.detailed_explanation, 4e4);
	if (mode === "concise") return conciseExplanation(explicitConcise || explicitDetailed || legacy);
	...
	if (question.knowledge_point) sections.push(`**考查要点：** ${cleanString$1(question.knowledge_point, 500)}`);
	const rubricPoints = question.scoring_rubric?.key_points?.map((item) => cleanString$1(item.description, 1e3)).filter(Boolean);
	if (rubricPoints?.length) sections.push(`**评分要点：**\n${rubricPoints.map((item) => `- ${item}`).join("\n")}`);
	return sections.filter(Boolean).join("\n\n") || "暂无解析。";
}
function responseGrounding(response, timestamp) {
	const evidence = response.rag_package?.evidence ?? [];
	const citations = response.rag_package?.citations ?? [];
	const meta = asRecord$2(response.final_output?.meta);
	const sourceRefs = uniqueStrings$1([
		...response.final_output?.evidence_refs ?? [],
		...evidence.map((item) => item.source_file ?? item.source_doc),
		...citations.map((item) => item.source_file ?? item.source_doc)
	]);
	const ragChunkIds = uniqueStrings$1([...evidence.map((item) => item.chunk_id), ...citations.map((item) => item.chunk_id)]);
	const rawConfidence = Number(response.rag_package?.confidence);
	return {
		query: cleanString$1(response.rag_package?.query, 4e3),
		sourceRefs,
		ragChunkIds,
		confidence: Number.isFinite(rawConfidence) ? Math.max(0, Math.min(1, rawConfidence)) : null,
		knowledgeBaseVersion: cleanString$1(response.rag_package?.knowledge_base_version ?? meta?.knowledge_base_version ?? meta?.corpus_version, 200) || null,
		retrievedAt: timestamp
	};
}
function createQuizSession(input) {
	...
	const questions = input.questions.map((rawQuestion, index) => {
		...
		return {
			...
			stem: cleanString$1(question.stem),
			options: question.options.map((option) => cleanString$1(option, 5e3)),
			answer: cleanString$1(question.answer, 5e3),
			reference_answer: cleanString$1(question.reference_answer, 2e4) || cleanString$1(question.answer, 2e4),
			explanation: cleanString$1(question.explanation),
			concise_explanation: cleanString$1(question.concise_explanation, 1e4) || void 0,
			detailed_explanation: cleanString$1(question.detailed_explanation, 4e4) || void 0,
			difficulty: cleanString$1(question.difficulty, 50) || input.difficulty,
			question_type: quizQuestionType(question),
			points: quizQuestionPoints(question),
			scoring_rubric: question.scoring_rubric,
			capability_dimension: cleanString$1(question.capability_dimension, 80) || void 0,
			knowledge_point: cleanString$1(question.knowledge_point, 300) || void 0,
			source_refs: uniqueStrings$1([...question.source_refs ?? [], ...retrieval.sourceRefs]),
			rag_chunk_ids: uniqueStrings$1([...question.rag_chunk_ids ?? [], ...retrieval.ragChunkIds]),
			source_grounding_scope: question.source_refs?.length || question.rag_chunk_ids?.length ? "question" : retrieval.sourceRefs.length || retrieval.ragChunkIds.length ? "session" : "none"
		};
	});
	return {
		...
		topic: cleanString$1(input.topic, 500),
		focus: cleanString$1(input.focus, 2e3),
		...
		assessmentLink: input.assessmentLink ? {
			lectureId: cleanString$1(input.assessmentLink.lectureId, 150),
			chapterId: cleanString$1(input.assessmentLink.chapterId, 150),
			objectiveIds: uniqueStrings$1(input.assessmentLink.objectiveIds)
		} : null
	};
}
function quizSessionProgress(session) {
	const submittedResponses = session.questions.map((question) => session.responses[question.id]).filter((response) => !!response?.submittedAt);
	const correct = submittedResponses.filter((response) => response.isCorrect === true).length;
	const earnedPoints = submittedResponses.reduce((sum, response) => sum + Number(response.earnedPoints || 0), 0);
	const submittedPossiblePoints = submittedResponses.reduce((sum, response) => sum + Number(response.possiblePoints || 0), 0);
	const paperPossiblePoints = session.questions.reduce((sum, question) => sum + quizQuestionPoints(question), 0);
	return {
		...
		completionRate: session.questions.length ? submittedResponses.length / session.questions.length : 0,
		accuracy: submittedResponses.length && submittedPossiblePoints ? earnedPoints / submittedPossiblePoints : null
	};
}
function updateQuizDraft(session, questionId, selectedAnswer, updatedAt = (/* @__PURE__ */ new Date()).toISOString()) {
	if (session.responses[questionId]?.submittedAt || !session.questions.some((item) => item.id === questionId)) return session;
	return {
		...session,
		responses: {
			...session.responses,
			[questionId]: {
				questionId,
				selectedAnswer: cleanString$1(selectedAnswer, 2e4),
				submittedAt: null,
				isCorrect: null,
				earnedPoints: null,
				possiblePoints: quizQuestionPoints(session.questions.find((item) => item.id === questionId)),
				gradingStatus: "draft",
				gradingMethod: "",
...
function submitQuizAnswer(session, questionId, gradingOrSubmittedAt, explicitSubmittedAt = (/* @__PURE__ */ new Date()).toISOString()) {
	const grading = typeof gradingOrSubmittedAt === "string" ? void 0 : gradingOrSubmittedAt;
	const submittedAt = typeof gradingOrSubmittedAt === "string" ? gradingOrSubmittedAt : explicitSubmittedAt;
	...
	const earnedPoints = grading ? Math.max(0, Math.min(possiblePoints, grading.earnedPoints)) : answerCorrect ? possiblePoints : 0;
	const nextResponses = {
		...session.responses,
		[questionId]: {
			...existing,
	...
	const completed = session.questions.every((item) => !!nextResponses[item.id]?.submittedAt);
	return {
		...session,
...
function setQuizExplanationMode(session, explanationMode, updatedAt = (/* @__PURE__ */ new Date()).toISOString()) {
	return {
		...session,
		explanationMode,
		updatedAt
...
function setQuizCurrentIndex(session, index, updatedAt = (/* @__PURE__ */ new Date()).toISOString()) {
...
function upsertQuizSession(sessions, session) {
	return [session, ...sessions.filter((item) => item.id !== session.id)].sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()).slice(0, 50);
}
function normalizeQuestion(value, sessionId, index) {
	const item = asRecord$2(value);
	if (!item || typeof item.stem !== "string") return null;
	const options = (Array.isArray(item.options) ? item.options : []).filter((option) => typeof option === "string").map((option) => cleanString$1(option, 5e3));
	const type = quizQuestionType(item);
	if ((type === "single_choice" || type === "true_false") && options.length < 2) return null;
	...
	return {
		id: cleanString$1(item.id, 150) || `${sessionId}-q${index + 1}`,
		stem: cleanString$1(item.stem),
		options: normalized.options,
		answer: cleanString$1(normalized.answer, 5e3),
		reference_answer: cleanString$1(normalized.reference_answer, 2e4) || cleanString$1(normalized.answer, 2e4),
		explanation: cleanString$1(item.explanation),
		concise_explanation: cleanString$1(item.concise_explanation, 1e4) || void 0,
		detailed_explanation: cleanString$1(item.detailed_explanation, 4e4) || void 0,
		difficulty: cleanString$1(item.difficulty, 50) || "easy",
		question_type: type,
		points: quizQuestionPoints(item),
		scoring_rubric: asRecord$2(item.scoring_rubric),
		capability_dimension: cleanString$1(item.capability_dimension, 80) || void 0,
		knowledge_point: cleanString$1(item.knowledge_point, 300) || void 0,
		source_refs: uniqueStrings$1(Array.isArray(item.source_refs) ? item.source_refs : []),
		rag_chunk_ids: uniqueStrings$1(Array.isArray(item.rag_chunk_ids) ? item.rag_chunk_ids : []),
		source_grounding_scope: item.source_grounding_scope === "question" || item.source_grounding_scope === "session" ? item.source_grounding_scope : "none"
	};
}
function normalizeQuizSessions(value) {
	if (!Array.isArray(value)) return [];
	const now = (/* @__PURE__ */ new Date()).toISOString();
	return value.flatMap((candidate) => {
		const item = asRecord$2(candidate);
		const id = cleanString$1(item?.id, 150);
		if (!item || !id || !Array.isArray(item.questions)) return [];
		const questions = item.questions.map((question, index) => normalizeQuestion(question, id, index)).filter((question) => !!question);
		if (!questions.length) return [];
		const responseRoot = asRecord$2(item.responses) ?? {};
		const responses = Object.fromEntries(questions.flatMap((question) => {
			const response = asRecord$2(responseRoot[question.id]);
			const selectedAnswer = cleanString$1(response?.selectedAnswer, 2e4);
			if (!selectedAnswer) return [];
			const submittedAt = cleanString$1(response?.submittedAt, 80) || null;
			return [[question.id, {
				questionId: question.id,
				selectedAnswer,
				submittedAt,
				isCorrect: submittedAt && typeof response?.isCorrect === "boolean" ? response.isCorrect : submittedAt ? selectedAnswer === quizAnswerKey(question) : null,
				earnedPoints: submittedAt ? Number.isFinite(Number(response?.earnedPoints)) ? Number(response?.earnedPoints) : selectedAnswer === quizAnswerKey(question) ? quizQuestionPoints(question) : 0 : null,
				possiblePoints: Number.isFinite(Number(response?.possiblePoints)) ? Number(response?.possiblePoints) : quizQuestionPoints(question),
				gradingStatus: submittedAt ? "graded" : "draft",
				gradingMethod: cleanString$1(response?.gradingMethod, 120) || (submittedAt ? "legacy_exact_answer_key" : ""),
				semanticSimilarity: Number.isFinite(Number(response?.semanticSimilarity)) ? Number(response?.semanticSimilarity) : null,
				keyPointCoverage: Number.isFinite(Number(response?.keyPointCoverage)) ? Number(response?.keyPointCoverage) : null,
				graderConfidence: Number.isFinite(Number(response?.graderConfidence)) ? Number(response?.graderConfidence) : null,
				feedback: cleanString$1(response?.feedback, 2e4),
				matchedKeyPoints: uniqueStrings$1(Array.isArray(response?.matchedKeyPoints) ? response.matchedKeyPoints : []),
				missedKeyPoints: uniqueStrings$1(Array.isArray(response?.missedKeyPoints) ? response.missedKeyPoints : []),
				rubricVersion: cleanString$1(response?.rubricVersion, 120),
				factualityScore: Number.isFinite(Number(response?.factualityScore)) ? Number(response?.factualityScore) : null,
				contradictions: uniqueStrings$1(Array.isArray(response?.contradictions) ? response.contradictions : []),
				safetyCriticalError: response?.safetyCriticalError === true,
				rubricPointScores: Array.isArray(response?.rubricPointScores) ? response.rubricPointScores.flatMap((value) => {
					const point = asRecord$2(value);
		...
		const createdAt = validDate$1(item.createdAt, now);
		const updatedAt = validDate$1(item.updatedAt, createdAt);
		const completed = questions.every((question) => !!responses[question.id]?.submittedAt);
		const retrievalRoot = asRecord$2(item.retrieval) ?? {};
		const assessmentLinkRoot = asRecord$2(item.assessmentLink);
		const rawConfidence = Number(retrievalRoot.confidence);
		return [{
			modelVersion: QUIZ_SESSION_MODEL_VERSION,
			id,
			requestId: cleanString$1(item.requestId, 200),
			courseId: cleanString$1(item.courseId, 200),
			chapterId: cleanString$1(item.chapterId, 200),
			topic: cleanString$1(item.topic, 500) || "未命名 Quiz",
			focus: cleanString$1(item.focus, 2e3),
			difficulty: difficulty(item.difficulty),
			status: item.status === "abandoned" ? "abandoned" : completed ? "completed" : "in_progress",
			generationStatus: item.generationStatus === "generating" ? "failed" : item.generationStatus === "failed" ? "failed" : "complete",
			expectedQuestionCount: Math.max(questions.length, Math.trunc(Number(item.expectedQuestionCount) || questions.length)),
			generationError: item.generationStatus === "generating" ? "上次题目生成在页面关闭或刷新时中断，已保留成功生成的题目。" : cleanString$1(item.generationError, 2e3),
			questions,
			responses,
			currentIndex: Math.max(0, Math.min(questions.length - 1, Math.trunc(Number(item.currentIndex) || 0))),
			explanationMode: item.explanationMode === "concise" ? "concise" : "detailed",
			createdAt,
			updatedAt,
			completedAt: completed ? validDate$1(item.completedAt, updatedAt) : null,
			retrieval: {
				query: cleanString$1(retrievalRoot.query, 4e3),
				sourceRefs: uniqueStrings$1(Array.isArray(retrievalRoot.sourceRefs) ? retrievalRoot.sourceRefs : []),
				ragChunkIds: uniqueStrings$1(Array.isArray(retrievalRoot.ragChunkIds) ? retrievalRoot.ragChunkIds : []),
				confidence: Number.isFinite(rawConfidence) ? Math.max(0, Math.min(1, rawConfidence)) : null,
				knowledgeBaseVersion: cleanString$1(retrievalRoot.knowledgeBaseVersion, 200) || null,
				retrievedAt: validDate$1(retrievalRoot.retrievedAt, createdAt)
			},
			assessmentLink: assessmentLinkRoot && cleanString$1(assessmentLinkRoot.lectureId, 150) ? {
				lectureId: cleanString$1(assessmentLinkRoot.lectureId, 150),
				chapterId: cleanString$1(assessmentLinkRoot.chapterId, 150),
				objectiveIds: uniqueStrings$1(Array.isArray(assessmentLinkRoot.objectiveIds) ? assessmentLinkRoot.objectiveIds : [])
			} : null
		}];
	}).sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()).slice(0, 50);
}
//#endregion
//#region lib/quiz-grading-client.ts
async function gradeSubjectiveQuizAnswer(input) {
	const controller = new AbortController();
	const timer = window.setTimeout(() => controller.abort(), 9e4);
	let response;
	try {
		response = await fetch("/api/quiz-grade", {
			...
			body: JSON.stringify({
				...
				reference_answer: input.question.reference_answer || input.question.answer,
		...
		if (error instanceof Error && error.name === "AbortError") throw new Error("主观题评分超时，请保留答案后重试");
	...
	if (!response.ok) {
		...
		const restartHint = response.status === 404 ? "。当前后端尚未加载评分接口，请关闭知链启动窗口后重新运行“启动前端.cmd”" : "";
		throw new Error((upstreamMessage || raw.slice(0, 240) || `主观题评分服务返回 HTTP ${response.status}`) + restartHint);
	}
	return {
		...
		semanticSimilarity: Number.isFinite(Number(data.semantic_similarity)) ? Number(data.semantic_similarity) : null,
		keyPointCoverage: Number.isFinite(Number(data.key_point_coverage)) ? Number(data.key_point_coverage) : null,
		graderConfidence: Number.isFinite(Number(data.grader_confidence)) ? Number(data.grader_confidence) : null,
		...
		factualityScore: Number.isFinite(Number(data.factuality_score)) ? Number(data.factuality_score) : null,
...
function allocateByRatio(total, ratios) {
	...
	const allocation = Object.fromEntries(entries.map(([key, ratio]) => [key, Math.floor(total * ratio)]));
	const remaining = total - Object.values(allocation).reduce((sum, value) => sum + value, 0);
	const ranked = entries.map(([key, ratio], index) => ({
		key,
		index,
		remainder: total * ratio - Math.floor(total * ratio)
	})).sort((a, b) => b.remainder - a.remainder || a.index - b.index);
	for (let index = 0; index < remaining; index += 1) allocation[ranked[index % ranked.length].key] += 1;
	return allocation;
}
function expandAllocation(allocation) {
	return Object.entries(allocation).flatMap(([key, count]) => Array.from({ length: count }, () => key));
}
function safeScore(scores, dimension) {
	const raw = Number(scores[dimension]);
	return Number.isFinite(raw) ? Math.max(0, Math.min(100, raw)) : 50;
}
function dimensionOrder(scores) {
	return [...CAPABILITY_DIMENSIONS].sort((a, b) => safeScore(scores, a.id) - safeScore(scores, b.id)).map((item) => item.id);
}
function allocateDimensions(total, scores) {
	const ordered = dimensionOrder(scores);
	const allocation = Object.fromEntries(ordered.map((dimension) => [dimension, 1]));
...
function orderedDifficulties(total) {
	...
	for (let index = 0; index < total; index += 1) {
		...
		const next = (progress < .35 ? [
			...
			"medium",
			"easy"
		]).find((item) => counts[item] > 0) ?? "easy";
		result.push(next);
		counts[next] -= 1;
	}
	return result;
}
function orderedTypes(total) {
	const counts = allocateByRatio(total, TYPE_RATIOS);
	return QUIZ_TYPE_ORDER.flatMap((type) => Array.from({ length: counts[type] }, () => type));
}
function orderedDifficultiesByType(types) {
	return QUIZ_TYPE_ORDER.flatMap((type) => {
		const count = types.filter((item) => item === type).length;
		return orderedDifficulties(count);
	});
}
function pointsFor(type, difficulty) {
	if (type === "single_choice" || type === "true_false") return difficulty === "hard" ? 2 : difficulty === "medium" ? 1.5 : 1;
	return difficulty === "hard" ? 12 : difficulty === "medium" ? 10 : 7;
}
function normalizeQuizCount(value) {
	const parsed = Math.trunc(Number(value));
	if (!Number.isFinite(parsed)) return 50;
	return Math.max(8, Math.min(50, parsed));
}
function createQuizBlueprint(requestedCount, scores) {
...
function summarizeQuizBlueprint(blueprint) {
	const byType = Object.fromEntries(Object.keys(TYPE_RATIOS).map((key) => [key, 0]));
	const byDifficulty = Object.fromEntries(Object.keys(DIFFICULTY_RATIOS).map((key) => [key, 0]));
	const byDimension = Object.fromEntries(CAPABILITY_DIMENSIONS.map((item) => [item.id, 0]));
...
function batchQuizBlueprint(blueprint, batchSize = 10) {
	const result = [];
	for (let index = 0; index < blueprint.length; index += batchSize) result.push(blueprint.slice(index, index + batchSize));
	return result;
}
//#endregion
//#region lib/backend-url.ts
var DEFAULT_AGENT_BASE_URL = "http://127.0.0.1:8000";
function agentBackendUrl(pathname) {
	const configuredBase = process.env.AGENT_API_BASE_URL?.trim();
	if (configuredBase) return new URL(pathname, `${configuredBase.replace(/\/$/, "")}/`).toString();
	const configuredGenerateUrl = process.env.AGENT_API_URL?.trim();
	if (configuredGenerateUrl) return new URL(pathname, new URL(configuredGenerateUrl).origin).toString();
	return new URL(pathname, `${DEFAULT_AGENT_BASE_URL}/`).toString();
}
//#endregion
//#region lib/storage-url.ts
function cleanStoragePath(storagePath) {
	return storagePath.replace(/^\/+/, "").trim();
}
function storageFileUrl(storagePath) {
	return agentBackendUrl(`/api/storage/files/${cleanStoragePath(storagePath)}`);
}
function storageMarkdownBaseUrl(storagePath) {
	return storageFileUrl(storagePath);
}
//#endregion
//#region lib/learning-progress.ts
var LEARNING_PROGRESS_MODEL_VERSION = "cnc-job-readiness-v2";
var JOB_READINESS_WEIGHTS = CNC_JOB_CAPABILITY_WEIGHTS;
var PROGRESS_DIMENSIONS = [
	{
		id: "safety",
		label: "安全规范"
	},
...
var COURSE_PHASES = [
	{
		...
		label: "行业入门",
		range: "1.1–1.5",
		outcome: "理解机床、加工、测量与质量基础"
	},
	{
		id: "l1",
		label: "L1 规范上机",
		range: "2.1–2.5",
		outcome: "具备安全上机与规范操作基础"
	},
	{
		id: "l2",
		label: "L2 独立加工",
		range: "3.1–3.5",
		outcome: "可完成典型单特征零件加工与自检"
	},
	{
		id: "l3",
		label: "L3 岗位综合",
		range: "4.1–4.5",
		outcome: "可完成工业性综合任务并作质量判定"
	}
];
...
function textFrom(record, keys) {
	return keys.map((key) => String(record[key] ?? "")).join(" ").trim();
}
function isResolvedGap(gap) {
	return /resolved|completed|closed|mastered|已解决|已掌握|完成/i.test(textFrom(gap, [
		"status",
		"state",
		"resolution"
	]));
}
function isSafetyGap(gap) {
	return /安全|急停|防护|违规|危险|safety/i.test(textFrom(gap, [
		"dimension",
		"topic",
		"knowledge_point",
		"description",
...
function verifiedAverageScores(assessment, ids, channel) {
	const values = ids.map((id) => {
		const result = assessment.dimensions[id];
		if (channel === "knowledge") return result.knowledgeStatus === "rated" ? result.knowledgeScore : null;
		return result.practiceStatus === "rated" ? result.practiceScore : null;
	}).filter((value) => value !== null);
	return values.length ? Math.round(values.reduce((sum, value) => sum + value, 0) / values.length) : null;
}
function ratedChannelCount(assessment, ids, channel) {
	return ids.filter((id) => {
		const result = assessment.dimensions[id];
		return channel === "knowledge" ? result.knowledgeStatus === "rated" : result.practiceStatus === "rated";
	}).length;
}
function scoreGate(id, label, value, threshold, blocking = true) {
	return {
		id,
		label,
...
function countGate(id, label, value, threshold, unit, blocking = true) {
	return {
		id,
		label,
...
function stage(id, label, roleOutcome, gates) {
...
function courseState(rows) {
	...
	for (const [chapterId] of COURSE_CHAPTERS) {
		...
		if (/in_progress|needs_review|进行中|复训/i.test(status)) currentChapterId = chapterId;
	}
...
function calculateLearningProgress(input) {
	...
	const practicalEvidenceCount = sourceEvidence.filter((item) => item.sourceType === "practice" && item.reviewStatus === "reviewed").length;
	const externalAssessmentCount = sourceEvidence.filter((item) => item.sourceType === "external_assessment" && item.reviewStatus === "reviewed").length;
	const quizEvidenceCount = sourceEvidence.filter((item) => item.sourceType === "quiz").length;
	const groundedEvidenceCount = sourceEvidence.filter((item) => item.questionGrounded).length;
	const openGaps = (input.knowledgeGaps ?? []).filter((gap) => !isResolvedGap(gap));
	const latestCriticalSafetyEvidence = sourceEvidence.filter((item) => item.dimension === "safety" && item.criticalSafetyError === true && item.reviewStatus !== "rejected").sort((a, b) => new Date(b.occurredAt).getTime() - new Date(a.occurredAt).getTime())[0];
	const safetyRecoveryAttempts = latestCriticalSafetyEvidence ? new Set(sourceEvidence.filter((item) => item.dimension === "safety" && item.criticalSafetyError !== true && item.reviewStatus !== "pending_review" && item.reviewStatus !== "rejected" && item.possible > 0 && item.earned / item.possible >= .8 && new Date(item.occurredAt).getTime() > new Date(latestCriticalSafetyEvidence.occurredAt).getTime()).map((item) => item.attemptId)).size : 0;
	const criticalSafetyGap = openGaps.some(isSafetyGap) || !!latestCriticalSafetyEvidence && safetyRecoveryAttempts < 2;
	const theoryDimensions = [
		"foundations",
		"process_planning",
		"programming",
		"quality_control"
	];
	const practicalDimensions = [
		"machining_operation",
		"quality_control",
		"maintenance"
	];
	const theoryReadiness = verifiedAverageScores(assessment, theoryDimensions, "knowledge");
	const practicalReadiness = verifiedAverageScores(assessment, practicalDimensions, "practice");
	const ratedTheoryDimensions = ratedChannelCount(assessment, theoryDimensions, "knowledge");
	const ratedPracticalDimensions = ratedChannelCount(assessment, practicalDimensions, "practice");
	const safetyKnowledgeScore = assessment.dimensions.safety.knowledgeStatus === "rated" ? assessment.dimensions.safety.knowledgeScore : null;
	const advancedKnowledgeScore = assessment.dimensions.advanced_manufacturing.knowledgeStatus === "rated" ? assessment.dimensions.advanced_manufacturing.knowledgeScore : null;
	...
	const jobReady = stage("job_ready", "岗位胜任", "达到项目内部就业准备标准，建议进入企业实岗复核", [
		...
		countGate("job-course", "课程完成度", course.completion, 90, "%"),
		{
			id: "job-safety-gap",
			label: "重大安全缺口",
			passed: !criticalSafetyGap,
			current: criticalSafetyGap ? "存在" : "未发现",
			requirement: "无未解决项",
			blocking: true
		}
	]);
	...
	const overallProgress = jobReady.passed ? 100 : Math.min(99, Math.round(completedBefore * 25 + current.completion * .25));
	const blockers = current.gates.filter((gate) => gate.blocking && !gate.passed).map((gate) => `${gate.label}：当前${gate.current}，要求${gate.requirement}`);
	const nextActions = blockers.slice(0, 3).map((item) => `优先补齐${item}`);
	if (!nextActions.length && openGaps.length) nextActions.push("复习并关闭当前 Memory 中尚未解决的知识缺口");
	const rawTraceCount = Math.max(0, input.chatQuestionCount) + Math.max(0, input.memoryEventCount) + Math.max(0, input.quizSessionCount) + practicalEvidenceCount + externalAssessmentCount;
...
//#endregion
//#region lib/lecture-session.ts
var LECTURE_SESSION_MODEL_VERSION = "zlink-lecture-session-v2";
var LECTURE_MASTERY_MODEL_VERSION = "zlink-lecture-mastery-v2";
function asRecord$1(value) {
	return value && typeof value === "object" ? value : null;
}
function cleanString(value, max = 1e5) {
	return typeof value === "string" ? value.trim().slice(0, max) : "";
}
...
function lectureCandidates(response) {
	const root = asRecord$1(response);
	const finalOutput = asRecord$1(root?.final_output);
	const finalMaterials = asRecord$1(root?.final_materials);
	const nestedMaterials = asRecord$1(finalOutput?.materials);
	return [
		...
		finalOutput
	].map(asRecord$1).filter((item) => !!item);
}
function parseSections(value) {
	if (!Array.isArray(value)) return [];
	return value.flatMap((candidate, index) => {
		const item = asRecord$1(candidate);
		const heading = cleanString(item?.heading ?? item?.title, 300);
		const content = cleanString(item?.content ?? item?.body, 5e4);
		if (!content) return [];
		return [{
			heading: heading || `第 ${index + 1} 节`,
			content
		}];
	});
}
function defaultDimensionsForChapter(chapterId) {
	return {
		"1.1": ["foundations"],
		"1.2": ["foundations"],
...
function nextCourseChapter(chapterId) {
	const index = COURSE_CHAPTERS.findIndex(([id]) => id === chapterId);
	const next = index >= 0 ? COURSE_CHAPTERS[index + 1] : void 0;
	return next ? {
		id: next[0],
		title: next[1]
	} : null;
}
function artifactPath(response) {
	const root = asRecord$1(response);
	const direct = asRecord$1(root?.lecture_artifact_paths);
	const saved = asRecord$1(root?.saved_lecture_artifact);
	const savedLecture = asRecord$1(asRecord$1(root?.saved_outputs)?.lecture);
	return cleanString(direct?.markdown ?? saved?.markdown_path ?? savedLecture?.markdown_path, 1e3);
}
function createLectureSession(input) {
	const timestamp = input.createdAt ?? (/* @__PURE__ */ new Date()).toISOString();
	let title = "";
	let summary = "";
	let sections = [];
	for (const output of lectureCandidates(input.response)) {
		const payload = asRecord$1(output.payload) ?? output;
		const parsed = parseSections(payload.sections ?? output.sections);
		const content = cleanString(payload.content ?? output.content, 1e5);
		if (!parsed.length && content) parsed.push({
			heading: "讲义正文",
			content
		});
		if (!parsed.length) continue;
		title = cleanString(output.title ?? payload.title, 500);
		summary = cleanString(output.summary ?? payload.summary, 2e3);
		sections = parsed;
		break;
	}
	if (!sections.length) throw new Error("中央调度器未返回符合接口协议的讲义内容");
	...
	const ragChunkIds = uniqueStrings([...evidence.map((item) => item.chunk_id), ...citations.map((item) => item.chunk_id)]);
	const confidence = Number(input.response.rag_package?.confidence);
	const chapterTitle = COURSE_CHAPTERS.find(([id]) => id === input.chapterId)?.[1] ?? input.chapterId;
	return {
		...
		objectiveIds: defaultDimensionsForChapter(input.chapterId).map((dimension) => `${input.chapterId}:${dimension}`),
		baselineEvidenceIds: uniqueStrings(input.capabilityEvidence.map((item) => item.id), 2e3),
		sourceRefs,
		ragChunkIds,
		ragConfidence: Number.isFinite(confidence) ? Math.max(0, Math.min(1, confidence)) : null,
		knowledgeBaseVersion: cleanString(input.response.rag_package?.knowledge_base_version, 200) || null,
...
function upsertLectureSession(sessions, session) {
	return [session, ...sessions.filter((item) => item.id !== session.id)].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, 50);
}
function calculateLectureMastery(lecture, evidence) {
	const linked = evidence.filter((item) => item.lectureId === lecture.id && item.chapterId === lecture.chapterId && (item.objectiveIds ?? []).some((objectiveId) => lecture.objectiveIds.includes(objectiveId)) && lecture.targetDimensions.includes(item.dimension));
	...
	const points = new Set(relevant.map((item) => item.knowledgePointId).filter(Boolean));
	...
	const practiceEvidence = relevant.filter((item) => item.sourceType === "practice" || item.sourceType === "external_assessment");
	...
	const safetyScore = safetyPossible ? Math.round(safetyEvidence.reduce((sum, item) => sum + item.earned, 0) / safetyPossible * 100) : null;
	const accuracyScore = accuracy === null ? null : Math.round(accuracy * 100);
	const confidence = relevant.length ? Math.min(1, Math.min(1, relevant.length / LECTURE_RATING_POLICY.minimumEffectiveEvidence) * .45 + Math.min(1, points.size / LECTURE_RATING_POLICY.minimumKnowledgePoints) * .25 + Math.min(1, independentAttempts.size / LECTURE_RATING_POLICY.minimumIndependentAttempts) * .2 + groundedCount / relevant.length * .1) : 0;
	const score = relevant.length >= LECTURE_RATING_POLICY.minimumEffectiveEvidence && points.size >= LECTURE_RATING_POLICY.minimumKnowledgePoints && independentAttempts.size >= LECTURE_RATING_POLICY.minimumIndependentAttempts && confidence >= LECTURE_RATING_POLICY.minimumConfidence ? accuracyScore : null;
	...
	const requirements = [
		...
		{
			label: "独立复测",
			passed: independentAttempts.size >= LECTURE_RATING_POLICY.minimumIndependentAttempts,
			current: `${independentAttempts.size} 次`,
			requirement: `≥ ${LECTURE_RATING_POLICY.minimumIndependentAttempts} 次`
		},
		{
			label: "相关题目正确率",
			passed: accuracy !== null && accuracy >= LECTURE_RATING_POLICY.minimumAccuracy,
		...
		...safetyRequired ? [{
			label: "安全知识正确率",
			passed: safetyScore !== null && safetyScore >= LECTURE_RATING_POLICY.minimumSafetyAccuracy * 100,
			current: safetyScore === null ? "待评估" : `${safetyScore}%`,
			requirement: `≥ ${Math.round(LECTURE_RATING_POLICY.minimumSafetyAccuracy * 100)}%`
		}] : [],
		...practiceRequired ? [{
			label: "已审核实操证据",
			passed: practiceEvidence.length >= LECTURE_RATING_POLICY.minimumReviewedPractice,
	...
	return {
		...
		status: relevant.length === 0 ? "not_assessed" : mastered ? "mastered" : "learning",
		score,
		...
		message: mastered ? "你已较好掌握当前知识，可以进一步进行学习啦" : relevant.length ? "当前作答已记录，但证据量、覆盖或独立复测尚未达标" : "尚无与本讲义目标精确关联的评价证据",
		requirements
	};
}
function normalizeLectureSessions(value) {
	if (!Array.isArray(value)) return [];
	const now = (/* @__PURE__ */ new Date()).toISOString();
	return value.flatMap((candidate) => {
		const item = asRecord$1(candidate);
		...
		const targetDimensions = Array.isArray(item.targetDimensions) ? item.targetDimensions.filter((dimension) => [
			"safety",
			"foundations",
			"process_planning",
			"programming",
			...
			"maintenance",
			"advanced_manufacturing"
		].includes(String(dimension))) : defaultDimensionsForChapter(chapterId);
		const reason = item.generationReason === "regenerate" || item.generationReason === "next_stage" ? item.generationReason : "initial";
		return [{
			modelVersion: LECTURE_SESSION_MODEL_VERSION,
			id,
			...
			chapterId,
			chapterTitle: cleanString(item.chapterTitle, 500) || COURSE_CHAPTERS.find(([chapter]) => chapter === chapterId)?.[1] || chapterId,
			title: cleanString(item.title, 500) || "未命名讲义",
			summary: cleanString(item.summary, 2e3),
			sections,
			targetDimensions: targetDimensions.length > 0 ? [...new Set(targetDimensions)] : defaultDimensionsForChapter(chapterId),
			objectiveIds: Array.isArray(item.objectiveIds) ? uniqueStrings(item.objectiveIds) : (targetDimensions.length > 0 ? [...new Set(targetDimensions)] : defaultDimensionsForChapter(chapterId)).map((dimension) => `${chapterId}:${dimension}`),
			baselineEvidenceIds: uniqueStrings(Array.isArray(item.baselineEvidenceIds) ? item.baselineEvidenceIds : [], 2e3),
			...
			ragConfidence: Number.isFinite(Number(item.ragConfidence)) ? Math.max(0, Math.min(1, Number(item.ragConfidence))) : null,
			knowledgeBaseVersion: cleanString(item.knowledgeBaseVersion, 200) || null,
			artifactPath: cleanString(item.artifactPath, 1e3),
...
//#endregion
//#region components/LearningWorkspace.tsx
var DEFAULT_PROFILE = {
	background: "非机械专业",
	level: "beginner",
	preference: "步骤化、少术语"
};
var DEFAULT_SCORES = assessmentToScoreMap(calculateCapabilityAssessment([]), "beginner");
var ACTIVE_USER_ID = "user_001";
var ACTIVE_COURSE_ID = "cnc_lathe";
var ACTIVE_CHAPTER_ID = "1.1";
var QA_CONTEXT_VERSION = "cnc-domain-v2";
var TRACE_LABELS = {
...
var INITIAL_MESSAGE = {
	id: "welcome-message",
	role: "assistant",
	content: "Hello，我是知链。你可以直接提问，也可以让我根据当前Memory生成个性化解释和测验"
};
function uid(prefix) {
...
function readLocalWorkspaceCache() {
	try {
...
function normalizeFrontendState(snapshot) {
	const messages = Array.isArray(snapshot.messages) ? snapshot.messages.map((item) => asRecord(item)).filter((item) => !!item && typeof item.id === "string" && (item.role === "user" || item.role === "assistant") && typeof item.content === "string").map((item) => ({
		id: item.id,
		role: item.role,
		content: item.content,
		...Array.isArray(item.followUps) ? { followUps: item.followUps.filter((value) => typeof value === "string") } : {}
	})) : void 0;
	const memoryEvents = Array.isArray(snapshot.memory_events) ? snapshot.memory_events.map((item) => asRecord(item)).filter((item) => !!item && typeof item.id === "string" && typeof item.title === "string" && typeof item.detail === "string" && typeof item.time === "string").map((item) => ({
		id: item.id,
		title: item.title,
		detail: item.detail,
		time: item.time
	...
	const userIdentity = identity && typeof identity.nickname === "string" && identity.nickname.trim() && typeof identity.avatarId === "string" && isProvidedAvatar(identity.avatarId) ? {
		nickname: identity.nickname.trim().slice(0, 20),
		avatarId: identity.avatarId
	} : void 0;
	const pendingSuggestions = Array.isArray(snapshot.pending_suggestions) ? snapshot.pending_suggestions.filter((item) => {
		const record = asRecord(item);
		return !!record && typeof record.id === "string" && record.kind === "profile" && !!asRecord(record.patch);
	}) : void 0;
	return {
		messages,
		history: Array.isArray(snapshot.history) ? snapshot.history.filter((item) => typeof item === "string" && !!item.trim()) : void 0,
		memoryEvents,
		pendingSuggestions,
		userIdentity,
		qaSessionId: snapshot.qa_context_version === QA_CONTEXT_VERSION && typeof snapshot.qa_session_id === "string" ? snapshot.qa_session_id : "",
		capabilityEvidence: asRecord(snapshot.capability_assessment) ? normalizeCapabilityEvidence(asRecord(snapshot.capability_assessment)?.evidence) : void 0,
		quizSessions: Array.isArray(snapshot.quiz_sessions) ? normalizeQuizSessions(snapshot.quiz_sessions) : void 0,
		activeQuizSessionId: typeof snapshot.active_quiz_session_id === "string" ? snapshot.active_quiz_session_id : "",
		lectureSessions: Array.isArray(snapshot.lecture_sessions) ? normalizeLectureSessions(snapshot.lecture_sessions) : void 0,
		activeLectureSessionId: typeof snapshot.active_lecture_session_id === "string" ? snapshot.active_lecture_session_id : ""
	};
}
function materialCandidates(response, kind) {
	...
	return [
		finalOutput,
		...(kind === "qa" ? ["personalized_qa_output", "final_qa_output"] : ["personalized_question_output", "final_question_output"]).map((key) => root?.[key]),
		finalMaterials?.[kind],
		nestedMaterials?.[kind]
	].map(asRecord).filter((item) => !!item);
}
function asQaPayload(response) {
	for (const output of materialCandidates(response, "qa")) {
		const payload = asRecord(output.payload) || output;
		const answer = payload.answer;
		if (typeof answer !== "string" || !answer.trim()) continue;
		const followUps = Array.isArray(payload.follow_ups) ? payload.follow_ups.filter((item) => typeof item === "string" && !!item.trim()) : void 0;
		return {
			question: typeof payload.question === "string" ? payload.question : void 0,
			answer,
...
function asQuizQuestions(response) {
	for (const output of materialCandidates(response, "quiz")) {
		const questions = (asRecord(output.payload) || output).questions;
		if (!Array.isArray(questions)) continue;
		const validQuestions = questions.filter((question) => !!question && typeof question === "object" && typeof question.stem === "string" && (Array.isArray(question.options) || ["cloze", "short_answer"].includes(String(question.question_type))));
		if (validQuestions.length) return validQuestions;
	}
	return [];
}
function applyBlueprintToQuestion(question, slot) {
	const questionType = slot.questionType;
	const options = Array.isArray(question.options) ? question.options : [];
	const normalized = normalizeTrueFalseQuestion({
		...question,
		question_type: questionType,
		options: questionType === "cloze" || questionType === "short_answer" ? [] : options,
	...
	if (questionType === "true_false" && !normalized.answer) throw new Error(`第 ${slot.sequence} 题的判断题答案无法识别，中央调度器必须返回 A/B、正确/错误或 true/false`);
	return normalized;
}
function mergeQuizResponses(responses, questions) {
	...
	return {
		...
		final_output: {
			...latest.final_output || {},
			payload: { questions },
			evidence_refs: [...new Set(responses.flatMap((response) => response.final_output?.evidence_refs ?? []))]
		},
		rag_package: {
			...latest.rag_package || {},
			evidence,
			citations,
			confidence: responses.length ? responses.reduce((sum, response) => sum + Number(response.rag_package?.confidence || 0), 0) / responses.length : 0
		},
		agent_trace: responses.flatMap((response) => response.agent_trace || [])
	};
}
function assessmentEvidenceForSession(session, onlyQuestionId) {
	return session.questions.flatMap((question) => {
		...
		return createQuizEvidence({
			...
			questions: [{
				...
				earned: response.earnedPoints ?? void 0,
				possible: response.possiblePoints,
				isCorrect: response.isCorrect ?? void 0,
...
function LearningWorkspace() {
	...
	const [userIdentity, setUserIdentity] = useState(DEFAULT_USER_IDENTITY);
	const [qaSessionId, setQaSessionId] = useState("");
	const [memoryBusy, setMemoryBusy] = useState(false);
	const [hydrated, setHydrated] = useState(false);
	const persistenceRevision = useRef(0);
	const capabilityAssessment = useMemo(() => calculateCapabilityAssessment(capabilityEvidence), [capabilityEvidence]);
	const scores = useMemo(() => assessmentToScoreMap(capabilityAssessment, profile.level), [capabilityAssessment, profile.level]);
	const learningProgress = useMemo(() => calculateLearningProgress({
	...
	const applyFrontendState = useCallback((snapshot) => {
		const restored = normalizeFrontendState(snapshot);
		if (restored.messages) setMessages(restored.messages.length ? restored.messages : [INITIAL_MESSAGE]);
		if (restored.history) setHistory(restored.history);
		if (restored.memoryEvents) setMemoryEvents(restored.memoryEvents);
		if (restored.pendingSuggestions) setPendingSuggestions(restored.pendingSuggestions);
		if (restored.userIdentity) setUserIdentity(restored.userIdentity);
		if (restored.capabilityEvidence) setCapabilityEvidence(restored.capabilityEvidence);
		if (restored.quizSessions) {
			setQuizSessions(restored.quizSessions);
			setActiveQuizSessionId(restored.quizSessions.some((session) => session.id === restored.activeQuizSessionId) ? restored.activeQuizSessionId : restored.quizSessions[0]?.id || "");
		}
		if (restored.lectureSessions) {
			setLectureSessions(restored.lectureSessions);
			setActiveLectureSessionId(restored.lectureSessions.some((session) => session.id === restored.activeLectureSessionId) ? restored.activeLectureSessionId : restored.lectureSessions[0]?.id || "");
		}
		setQaSessionId(restored.qaSessionId);
	}, []);
	const learningRecommendations = useMemo(() => buildLearningRecommendations({
		profile,
		scores,
		memoryEvents,
		ragPackage: lastResponse?.rag_package
	}), [
		lastResponse?.rag_package,
		memoryEvents,
		profile,
		scores
	]);
	useEffect(() => {
		...
		Promise.all([checkBackendHealth(), loadBackendWorkspaceState(ACTIVE_USER_ID)]).then(([health, backendState]) => {
			if (cancelled) return;
			setConnection(health.model_configured ? "live" : "needs-key");
			setProfile(mergeBackendProfile(backendState, cached?.profile || DEFAULT_PROFILE, cached?.scores || DEFAULT_SCORES).profile);
			setBackendKnowledgeGaps(backendState.knowledge_gaps ?? []);
			setBackendCourseProgress(backendState.learning_progress ?? []);
			persistenceRevision.current = Math.max(persistenceRevision.current, Number(backendState.client_revision || 0));
			const remoteState = backendState.frontend_state;
			if (remoteState && Object.keys(remoteState).length) {
				applyFrontendState(remoteState);
				if (!asRecord(remoteState.capability_assessment) && cached?.frontendState && asRecord(cached.frontendState.capability_assessment)) setCapabilityEvidence(normalizeCapabilityEvidence(asRecord(cached.frontendState.capability_assessment)?.evidence));
				if (!Array.isArray(remoteState.quiz_sessions) && cached?.frontendState && Array.isArray(cached.frontendState.quiz_sessions)) {
					const cachedSessions = normalizeQuizSessions(cached.frontendState.quiz_sessions);
					const requestedSessionId = typeof cached.frontendState.active_quiz_session_id === "string" ? cached.frontendState.active_quiz_session_id : "";
					setQuizSessions(cachedSessions);
					setActiveQuizSessionId(cachedSessions.some((session) => session.id === requestedSessionId) ? requestedSessionId : cachedSessions[0]?.id || "");
				}
				if (!Array.isArray(remoteState.lecture_sessions) && cached?.frontendState && Array.isArray(cached.frontendState.lecture_sessions)) {
					const cachedLectures = normalizeLectureSessions(cached.frontendState.lecture_sessions);
					const requestedLectureId = typeof cached.frontendState.active_lecture_session_id === "string" ? cached.frontendState.active_lecture_session_id : "";
					setLectureSessions(cachedLectures);
					setActiveLectureSessionId(cachedLectures.some((lecture) => lecture.id === requestedLectureId) ? requestedLectureId : cachedLectures[0]?.id || "");
				}
			} else if (cached?.frontendState) applyFrontendState(cached.frontendState);
		}).catch(() => {
			if (cancelled) return;
			setConnection("idle");
	...
	useEffect(() => {
		...
		const frontendState = {
			...
			capability_assessment: {
				...
				evidence: capabilityEvidence.slice(0, 2e3)
			},
		...
		try {
			window.localStorage.setItem("knowledge-chain-memory-v1", JSON.stringify({
				profile,
				scores,
				frontendState
			}));
		} catch {}
		const clientRevision = Math.max(Date.now(), persistenceRevision.current + 1);
		persistenceRevision.current = clientRevision;
		const timer = window.setTimeout(() => {
			saveBackendWorkspaceState(ACTIVE_USER_ID, {
				profile,
				scores,
	...
	function addSuggestions(response) {
		const suggestions = [];
		for (const patch of response.profile_update_suggestions?.md_patches || []) suggestions.push({
			id: uid("profile"),
			kind: "profile",
			patch
		});
		if (suggestions.length) setPendingSuggestions((current) => [...suggestions, ...current].slice(0, 8));
	}
	async function runAgent(request) {
		...
		try {
			...
			const message = error instanceof Error ? error.message : "无法连接中央调度器";
	...
	async function refreshMemory(showToast = true) {
		...
		try {
			...
			setProfile(mergeBackendProfile(backendState, profile, scores).profile);
			setBackendKnowledgeGaps(backendState.knowledge_gaps ?? []);
			setBackendCourseProgress(backendState.learning_progress ?? []);
			persistenceRevision.current = Math.max(persistenceRevision.current, Number(backendState.client_revision || 0));
			if (showToast && backendState.frontend_state && Object.keys(backendState.frontend_state).length) applyFrontendState(backendState.frontend_state);
	...
	async function saveMemory() {
		setMemoryBusy(true);
		try {
			setProfile(mergeBackendProfile(await saveBackendProfile(ACTIVE_USER_ID, profile, scores), profile, scores).profile);
			...
			/* @__PURE__ */ jsxs("main", {
				...
				children: [/* @__PURE__ */ jsxs("header", {
					...
					children: [/* @__PURE__ */ jsxs("div", {
						...
						children: [/* @__PURE__ */ jsx("h1", { children: pageMeta.title }), /* @__PURE__ */ jsx("p", { children: pageMeta.detail })]
					}), /* @__PURE__ */ jsx("span", {
						className: "orchestrator-badge",
						children: "中央调度器 · HTTP v1"
					})]
				}), /* @__PURE__ */ jsxs("div", {
					className: "page-content",
					children: [
						activeView === "chat" && /* @__PURE__ */ jsx(ChatView, {
							messages,
							setMessages,
							setHistory,
							busy,
						...
						activeView === "quiz" && /* @__PURE__ */ jsx(QuizView, {
							busy,
							profile,
							scores,
							recommendations: learningRecommendations,
							...
							activeLecture: lectureSessions.find((lecture) => lecture.id === activeLectureSessionId) ?? null,
							activeSessionId: activeQuizSessionId,
							onActiveSessionChange: setActiveQuizSessionId,
							onSessionChange: (session) => setQuizSessions((current) => upsertQuizSession(current, session)),
							onQuestionSubmitted: (session, questionId) => {
								const evidence = assessmentEvidenceForSession(session, questionId);
								setCapabilityEvidence((current) => mergeCapabilityEvidence(current, evidence));
							},
							onFinished: async (session) => {
								const evidence = assessmentEvidenceForSession(session);
								const nextEvidence = mergeCapabilityEvidence(capabilityEvidence, evidence);
								const nextAssessment = calculateCapabilityAssessment(nextEvidence);
								const nextScores = assessmentToScoreMap(nextAssessment, profile.level);
								setCapabilityEvidence(nextEvidence);
								const progress = quizSessionProgress(session);
								const changedDimensions = [...new Set(evidence.map((item) => nextAssessment.dimensions[item.dimension].shortLabel))];
						...
						activeView === "lecture" && /* @__PURE__ */ jsx(LectureView, {
						...
						activeView === "memory" && /* @__PURE__ */ jsx(MemoryView, {
							profile,
							assessment: capabilityAssessment,
							progress: learningProgress,
							setProfile,
						...
						activeView === "progress" && /* @__PURE__ */ jsx(LearningProgressView, {
							progress: learningProgress,
							profile,
							scores,
							busy,
...
function Sidebar({ activeView, setActiveView, history, connection, userIdentity, setUserIdentity }) {
		...
		children: [
			/* @__PURE__ */ jsxs("div", {
				...
				children: [/* @__PURE__ */ jsx("span", {
					...
					children: [/* @__PURE__ */ jsx("strong", { children: "知链" }), /* @__PURE__ */ jsx("small", { children: "ZLink" })]
				})]
			}),
			/* @__PURE__ */ jsx("nav", {
				className: "nav-list",
				...
				children: [
					...
					"aria-current": activeView === item.id ? "page" : void 0,
			...
			/* @__PURE__ */ jsxs("section", {
				...
				children: [/* @__PURE__ */ jsx("p", {
					...
					children: history.length ? history.slice(0, 7).map((item, index) => /* @__PURE__ */ jsx("button", {
						type: "button",
						className: "history-item",
						title: item,
						onClick: () => setActiveView("chat"),
						children: item
					}, `${item}-${index}`)) : /* @__PURE__ */ jsx("div", {
						className: "history-item",
						children: "发送问题后显示记录"
					})
				})]
			...
			/* @__PURE__ */ jsxs("footer", {
				...
				children: [/* @__PURE__ */ jsxs("div", {
					...
					children: [/* @__PURE__ */ jsx("span", { className: `status-dot ${connection}` }), /* @__PURE__ */ jsx("span", { children: connectionText(connection) })]
				}), /* @__PURE__ */ jsx(UserProfileControl, {
...
function ChatView({ messages, setMessages, setHistory, busy, profile, scores, memoryEvents, runAgent, trace, response, userIdentity, qaSessionId, setQaSessionId, recommendations }) {
	...
	async function sendQuestion(prompt) {
		...
		const request = buildAgentRequest({
			...
			profile: {
				...
				recent_memory: memoryEvents.slice(0, 6).map((event) => `${event.title}：${event.detail}`)
			},
			qaSessionId: qaSessionId || void 0
		});
		try {
			const result = await runAgent(request);
			if (result.qa_session_id) setQaSessionId(result.qa_session_id);
			const payload = asQaPayload(result);
			const content = payload?.answer || result.final_output?.summary || "中央调度器已完成任务，但没有返回可展示的问答内容。";
			setMessages((items) => [...items, {
				id: uid("assistant"),
				role: "assistant",
				content,
			...
			setMessages((items) => [...items, {
				...
				role: "assistant",
				content: error instanceof Error ? `本次请求失败：${error.message}` : "本次请求失败，请检查后端服务。"
			}]);
		}
	}
	...
	return /* @__PURE__ */ jsxs("div", {
		className: `chat-layout ${showWelcome ? "welcome-layout" : ""}`,
		children: [/* @__PURE__ */ jsxs("section", {
			className: `chat-panel ${showWelcome ? "welcome-state" : ""}`,
			children: [/* @__PURE__ */ jsx("div", {
				...
				children: /* @__PURE__ */ jsxs("div", {
					className: "messages",
					children: [
						showWelcome && /* @__PURE__ */ jsxs("div", {
							className: "welcome",
							children: [
								/* @__PURE__ */ jsx("h2", { children: "今天想从哪个知识点开始？" }),
								/* @__PURE__ */ jsxs("div", {
									className: "welcome-intro",
									children: [/* @__PURE__ */ jsx("span", {
										className: "welcome-logo-shell",
										"aria-hidden": "true",
										children: /* @__PURE__ */ jsx("img", {
											src: "/zhilian-logo.png",
											alt: "",
									...
									}), /* @__PURE__ */ jsx("p", { children: INITIAL_MESSAGE.content })]
								}),
								/* @__PURE__ */ jsx("p", {
									className: "recommendation-context",
									"aria-live": "polite",
									children: recommendations.contextLabel
								}),
								/* @__PURE__ */ jsx("div", {
									className: "prompt-grid",
									children: recommendations.chatPrompts.map((prompt) => /* @__PURE__ */ jsx("button", {
										type: "button",
						...
						!showWelcome && messages.map((message) => /* @__PURE__ */ jsxs("article", {
							className: `message ${message.role}`,
							children: [/* @__PURE__ */ jsx("div", {
								className: "message-avatar",
								children: message.role === "assistant" ? /* @__PURE__ */ jsx("img", {
								...
								children: [
									/* @__PURE__ */ jsx("p", {
										...
										children: message.role === "assistant" ? "知链助手" : userIdentity.nickname
									}),
									message.role === "assistant" ? /* @__PURE__ */ jsx(MarkdownContent, { content: message.content }) : /* @__PURE__ */ jsx("div", {
										className: "message-text",
										children: message.content
									}),
									!!message.followUps?.length && /* @__PURE__ */ jsx("div", {
										className: "followups",
										children: message.followUps.map((followUp) => /* @__PURE__ */ jsx("button", {
											type: "button",
					...
					children: [/* @__PURE__ */ jsx("textarea", {
						...
						children: [/* @__PURE__ */ jsxs("div", {
							...
							children: [/* @__PURE__ */ jsx("span", {
								className: "mini-chip",
								children: "QA"
							}), /* @__PURE__ */ jsx("span", { children: profile.preference })]
						}), /* @__PURE__ */ jsx("button", {
							type: "submit",
		...
		}), !showWelcome && /* @__PURE__ */ jsx(AgentActivity, {
			busy,
			trace,
			response
		})]
...
function AgentActivity({ busy, trace, response }) {
	...
	return /* @__PURE__ */ jsxs("aside", {
		...
		children: [
			/* @__PURE__ */ jsxs("div", {
				...
				children: [/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("h3", { children: "Agent 活动" }), /* @__PURE__ */ jsx("p", { children: busy ? "任务执行中" : trace.length ? "最近一次执行轨迹" : "等待任务" })] }), !!response && /* @__PURE__ */ jsx("span", {
					className: "mini-chip",
					children: "v1"
				})]
			}),
			displayed.length ? /* @__PURE__ */ jsx("div", {
				className: "trace-list",
				children: displayed.map((item, index) => {
					const state = item.status === "running" ? "running" : item.status === "queued" ? "queued" : "done";
					return /* @__PURE__ */ jsxs("div", {
						className: `trace-item ${state}`,
						children: [/* @__PURE__ */ jsx("span", {
							className: "trace-node",
							children: state === "done" ? "✓" : state === "running" ? "•" : index + 1
						}), /* @__PURE__ */ jsxs("div", {
							className: "trace-copy",
							children: [/* @__PURE__ */ jsx("strong", { children: TRACE_LABELS[item.node] || item.node }), /* @__PURE__ */ jsx("p", { children: item.summary })]
						})]
					}, `${item.node}-${index}`);
				})
			}) : /* @__PURE__ */ jsx("div", {
				className: "empty-activity",
				children: "发送问题后，这里会展示中央调度器返回的 agent_trace。"
			}),
			!!response && /* @__PURE__ */ jsxs("div", {
				className: "report-card",
				children: [
					/* @__PURE__ */ jsx("h4", { children: "质量报告" }),
					/* @__PURE__ */ jsxs("div", {
						className: "metric-row",
						children: [/* @__PURE__ */ jsx("span", { children: "任务状态" }), /* @__PURE__ */ jsx("strong", { children: response.status })]
					}),
					/* @__PURE__ */ jsxs("div", {
						className: "metric-row",
						children: [/* @__PURE__ */ jsx("span", { children: "证据覆盖率" }), /* @__PURE__ */ jsxs("strong", { children: [Math.round((response.check_report?.evidence_coverage || 0) * 100), "%"] })]
					}),
					/* @__PURE__ */ jsxs("div", {
						className: "metric-row",
						children: [/* @__PURE__ */ jsx("span", { children: "RAG 置信度" }), /* @__PURE__ */ jsxs("strong", { children: [Math.round((response.rag_package?.confidence || 0) * 100), "%"] })]
					}),
					/* @__PURE__ */ jsxs("div", {
						className: "metric-row",
						children: [/* @__PURE__ */ jsx("span", { children: "安全审查" }), /* @__PURE__ */ jsx("strong", { children: response.safety_report?.status || "—" })]
					})
				]
			}),
			!!ragEvidence.length && /* @__PURE__ */ jsxs("div", {
				className: "rag-source-card",
				children: [/* @__PURE__ */ jsxs("div", {
					className: "rag-source-heading",
					children: [/* @__PURE__ */ jsx("h4", { children: "知识库依据" }), /* @__PURE__ */ jsx("span", { children: response?.rag_package?.knowledge_base_version || "当前版本" })]
				}), /* @__PURE__ */ jsx("div", {
					className: "rag-source-list",
					children: ragEvidence.slice(0, 4).map((item, index) => {
						const source = item.source_file ?? item.source_doc ?? "未知来源";
						return /* @__PURE__ */ jsxs("div", {
							className: "rag-source-item",
							children: [/* @__PURE__ */ jsx("strong", {
								title: source,
								children: source.split(/[\\/]/).pop()
...
function QuizView({ busy, profile, scores, recommendations, runAgent, sessions, activeLecture, activeSessionId, onActiveSessionChange, onSessionChange, onQuestionSubmitted, onFinished }) {
	...
	const recommendedDifficulty = profile.level === "advanced" ? "hard" : profile.level === "intermediate" ? "medium" : "easy";
	...
	const [panel, setPanel] = useState(activeSessionId ? "current" : "history");
	const activeSession = sessions.find((session) => session.id === activeSessionId) ?? null;
	const displayedQuizError = quizError || activeSession?.generationError || null;
	const generationRunning = !!generationProgress || activeSession?.generationStatus === "generating";
	const blueprint = useMemo(() => createQuizBlueprint(count, scores), [count, scores]);
	const blueprintSummary = useMemo(() => summarizeQuizBlueprint(blueprint), [blueprint]);
	async function generateQuiz(event) {
		event.preventDefault();
		if (generationLock.current) return;
		generationLock.current = true;
		setQuizError(null);
		let latestPartialSession = null;
		try {
			...
			for (let batchIndex = 0; batchIndex < batches.length; batchIndex += 1) {
				...
				setGenerationProgress(`正在生成第 ${batchIndex + 1}/${batches.length} 批 · 已完成 ${nextQuestions.length}/${blueprint.length} 题`);
				const prompt = [
				...
				const response = await runAgent(buildAgentRequest({
					userId: ACTIVE_USER_ID,
					courseId: ACTIVE_COURSE_ID,
					chapterId: linkToCurrentLecture && activeLecture ? activeLecture.chapterId : ACTIVE_CHAPTER_ID,
					prompt,
					contentType: "quiz",
					scores,
					profile
				...
				if (generated.length < batch.length) throw new Error(`中央调度器第 ${batchIndex + 1} 批只返回 ${generated.length}/${batch.length} 道有效题目，请重新生成`);
				responses.push(response);
				nextQuestions.push(...batch.map((slot, index) => applyBlueprintToQuestion(generated[index], slot)));
				...
				const partialSession = {
					...createQuizSession({
						...
						assessmentLink: linkToCurrentLecture && activeLecture ? {
							lectureId: activeLecture.id,
							chapterId: activeLecture.chapterId,
							objectiveIds: activeLecture.targetDimensions.map((dimension) => `${activeLecture.chapterId}:${dimension}`)
						} : null
					}),
					generationStatus: nextQuestions.length < blueprint.length ? "generating" : "complete",
				...
				setPanel("current");
				setGenerationProgress(nextQuestions.length < blueprint.length ? `已生成 ${nextQuestions.length}/${blueprint.length} 题，继续生成下一批…` : `已生成全部 ${blueprint.length} 题，正在保存…`);
			}
		} catch (error) {
			const message = error instanceof Error ? error.message : "无法连接中央调度器，请先接入后端接口";
			const preserved = latestPartialSession;
			if (preserved) onSessionChange({
				...preserved,
				generationStatus: "failed",
				generationError: message,
				updatedAt: (/* @__PURE__ */ new Date()).toISOString()
			});
			setQuizError(preserved ? `${message}。已保留 ${preserved.questions.length}/${preserved.expectedQuestionCount} 道成功生成的题目` : message);
		} finally {
	...
	const response = question ? activeSession?.responses[question.id] : void 0;
	const selectedAnswer = response?.selectedAnswer;
	const isSubmitted = !!response?.submittedAt;
	const progress = activeSession ? quizSessionProgress(activeSession) : {
		submitted: 0,
		total: 0,
		correct: 0,
		accuracy: null
	...
	async function submitAnswer() {
		...
		if (next.status === "completed" && activeSession.status !== "completed") onFinished(next);
	}
	return /* @__PURE__ */ jsx("div", {
		...
		children: /* @__PURE__ */ jsxs("div", {
			...
			children: [/* @__PURE__ */ jsxs("div", {
				...
				children: [/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("h2", { children: "生成个性化 Quiz" }), /* @__PURE__ */ jsx("p", { children: "配置测验目标后，前端会以 content_type=quiz 向中央调度器发起请求。题目、作答进度和知识来源会自动保存。" })] }), /* @__PURE__ */ jsxs("span", {
					className: "orchestrator-badge",
					children: ["画像水平 · ", profile.level]
				})]
			}), /* @__PURE__ */ jsxs("div", {
				className: "quiz-grid",
				children: [/* @__PURE__ */ jsxs("form", {
					...
					children: [
						...
						activeLecture && /* @__PURE__ */ jsxs("label", {
							className: "assessment-link-toggle",
							children: [/* @__PURE__ */ jsx("input", {
								type: "checkbox",
								checked: linkToCurrentLecture,
								onChange: (event) => setLinkToCurrentLecture(event.target.checked)
							}), /* @__PURE__ */ jsxs("span", { children: [/* @__PURE__ */ jsx("strong", { children: "用于评估当前讲义" }), /* @__PURE__ */ jsxs("small", { children: [
								"精确关联 ",
						...
						/* @__PURE__ */ jsxs("div", {
							className: "quiz-recommendations",
							children: [/* @__PURE__ */ jsxs("div", {
								className: "quiz-recommendations-head",
								children: [/* @__PURE__ */ jsx("span", { children: "根据 Memory 推荐" }), /* @__PURE__ */ jsx("small", { children: recommendations.contextLabel })]
							}), /* @__PURE__ */ jsx("div", {
								className: "quiz-recommendation-list",
								children: recommendations.quizOptions.map((option) => /* @__PURE__ */ jsxs("button", {
									type: "button",
									className: `quiz-recommendation-chip ${topic === option.topic ? "active" : ""}`,
									title: option.reason,
									onClick: () => {
										setTopicOverride(option.topic);
										setFocusOverride(option.focus);
									},
									children: [/* @__PURE__ */ jsx("strong", { children: option.topic }), /* @__PURE__ */ jsx("span", { children: option.reason })]
								}, option.id))
							})]
						}),
						/* @__PURE__ */ jsxs("div", {
						...
						/* @__PURE__ */ jsxs("div", {
							className: "quiz-blueprint-card",
							children: [/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("strong", { children: "本次测验蓝图" }), /* @__PURE__ */ jsxs("span", { children: [
								blueprintSummary.total,
								" 题 · ",
								blueprintSummary.totalPoints,
								" 分原始分"
							] })] }), /* @__PURE__ */ jsxs("div", {
								className: "quiz-blueprint-tags",
								children: [
									Object.entries(blueprintSummary.byType).map(([type, value]) => /* @__PURE__ */ jsxs("span", { children: [
										QUIZ_TYPE_LABELS[type],
										" ",
										value
									] }, type)),
									/* @__PURE__ */ jsx("span", { children: "难度 3:5:2" }),
									/* @__PURE__ */ jsx("span", { children: "八维全覆盖" })
								]
							})]
						}),
						/* @__PURE__ */ jsxs("div", {
						...
						/* @__PURE__ */ jsx("button", {
							...
							children: generationProgress || (generationRunning ? "题目正在生成…" : busy ? "中央调度器处理中…" : `生成 ${blueprintSummary.total} 题 Quiz`)
						})
					]
				}), /* @__PURE__ */ jsxs("section", {
					className: "card quiz-result-card",
					children: [/* @__PURE__ */ jsxs("div", {
						className: "quiz-panel-tabs",
						role: "tablist",
						"aria-label": "Quiz 视图",
						children: [/* @__PURE__ */ jsxs("button", {
							type: "button",
							...
							children: ["当前测验", activeSession && /* @__PURE__ */ jsxs("span", { children: [
								progress.submitted,
								"/",
								progress.total
							] })]
							...
							children: ["历史记录 ", /* @__PURE__ */ jsx("span", { children: sessions.length })]
						})]
					}), panel === "history" ? /* @__PURE__ */ jsx(QuizHistoryPanel, {
						sessions,
						onOpen: (sessionId) => {
							onActiveSessionChange(sessionId);
							setPanel("current");
						}
					}) : !question || !activeSession ? /* @__PURE__ */ jsx("div", {
						className: `quiz-empty ${displayedQuizError ? "has-error" : ""} ${generationRunning ? "is-generating" : ""}`,
						"aria-live": "polite",
						children: /* @__PURE__ */ jsxs("div", { children: [
							/* @__PURE__ */ jsx("span", {
								className: "quiz-empty-symbol",
								children: "测"
							}),
							/* @__PURE__ */ jsx("strong", { children: displayedQuizError ? "Quiz 暂不可用" : generationRunning ? "知链正在生成题目" : "等待后端生成测验" }),
							/* @__PURE__ */ jsx("p", { children: displayedQuizError || generationProgress || (generationRunning ? `已生成 ${activeSession?.questions.length || 0}/${activeSession?.expectedQuestionCount || blueprint.length} 题，正在继续生成…` : "") || "完成左侧设置并生成 Quiz。生成后会立即保存，刷新页面也能继续作答。" })
						] })
					}) : /* @__PURE__ */ jsxs(Fragment$1, { children: [
						/* @__PURE__ */ jsx(QuizQuestionOverview, {
							session: activeSession,
							onOpen: moveToQuestion
						}),
						generationRunning && /* @__PURE__ */ jsxs("div", {
							className: "quiz-generation-banner",
							role: "status",
							"aria-live": "polite",
							children: [/* @__PURE__ */ jsx("span", {
								className: "quiz-generation-spinner",
								"aria-hidden": "true"
							}), /* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("strong", { children: "题目正在分批生成" }), /* @__PURE__ */ jsx("span", { children: generationProgress || `已生成 ${activeSession.questions.length}/${activeSession.expectedQuestionCount} 题，正在继续生成…` })] })]
						}),
						/* @__PURE__ */ jsxs("div", {
							...
							children: [/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsxs("span", {
								...
								className: `quiz-session-status ${activeSession.status}`,
								children: activeSession.status === "completed" ? "已完成" : "自动保存中"
							})] }), /* @__PURE__ */ jsxs("div", {
								className: "quiz-source-summary",
								children: [!!activeSession.retrieval.sourceRefs.length && /* @__PURE__ */ jsxs("span", { children: [activeSession.retrieval.sourceRefs.length, " 个知识来源"] }), /* @__PURE__ */ jsxs("span", {
									className: "difficulty-chip",
									children: [
										QUIZ_TYPE_LABELS[quizQuestionType(question)],
										" · ",
										QUIZ_DIFFICULTY_LABELS[question.difficulty === "hard" ? "hard" : question.difficulty === "medium" ? "medium" : "easy"],
										" · ",
						...
						isSubjectiveQuizQuestion(question) ? /* @__PURE__ */ jsxs("div", {
							...
							children: [
								...
								/* @__PURE__ */ jsx("textarea", {
									...
									disabled: isSubmitted || generationRunning || gradingQuestionId === question.id,
									placeholder: quizQuestionType(question) === "cloze" ? "填写被挖空的核心概念，可使用等价表述" : "按要点作答，说明原理、步骤或判断依据",
									onChange: (event) => selectAnswer(event.target.value)
								}),
								/* @__PURE__ */ jsx("small", { children: "提交后由中央评分 Agent 结合评分点、语义向量和安全规则给出部分分。" })
							]
						}) : /* @__PURE__ */ jsx("div", {
							className: `option-list ${quizQuestionType(question) === "true_false" ? "true-false-list" : ""}`,
							role: quizQuestionType(question) === "true_false" ? "radiogroup" : void 0,
							"aria-label": quizQuestionType(question) === "true_false" ? "选择正确或错误" : void 0,
							children: question.options.map((option, optionIndex) => {
								...
								return /* @__PURE__ */ jsxs("button", {
									type: "button",
									className: `option-button ${isSubmitted ? key === currentAnswer ? "correct" : key === selectedAnswer ? "wrong" : "" : selectedAnswer === key ? "selected" : ""}`,
									role: isTrueFalse ? "radio" : void 0,
									"aria-checked": isTrueFalse ? selectedAnswer === key : void 0,
									disabled: isSubmitted || generationRunning,
									onClick: () => selectAnswer(key),
									children: [/* @__PURE__ */ jsx("span", {
										className: "option-key",
										children: isTrueFalse ? optionIndex === 0 ? "✓" : "×" : key
									}), /* @__PURE__ */ jsx("span", { children: option })]
								}, `${key}-${option}`);
							})
						}),
						isSubmitted && /* @__PURE__ */ jsxs("div", {
							className: "explanation",
							children: [
								/* @__PURE__ */ jsxs("div", {
									className: "explanation-heading",
									children: [/* @__PURE__ */ jsx("strong", { children: isSubjectiveQuizQuestion(question) ? `得分 ${response?.earnedPoints ?? 0}/${response?.possiblePoints ?? quizQuestionPoints(question)}` : selectedAnswer === currentAnswer ? "回答正确。" : `正确答案：${currentAnswer}。` }), /* @__PURE__ */ jsxs("div", {
										...
										children: [/* @__PURE__ */ jsx("button", {
											...
											onClick: () => updateActive((session) => setQuizExplanationMode(session, "concise")),
											children: "简洁解析"
										}), /* @__PURE__ */ jsx("button", {
											type: "button",
											className: activeSession.explanationMode === "detailed" ? "active" : "",
											"aria-pressed": activeSession.explanationMode === "detailed",
											onClick: () => updateActive((session) => setQuizExplanationMode(session, "detailed")),
											children: "详细解析"
										})]
									})]
								}),
								/* @__PURE__ */ jsx("div", {
									...
									"aria-live": "polite",
									children: /* @__PURE__ */ jsx(MarkdownContent, { content: quizExplanationText(question, activeSession.explanationMode) })
								}),
								response?.feedback && /* @__PURE__ */ jsxs("div", {
									className: `grading-feedback ${response.safetyCriticalError ? "critical" : ""}`,
									children: [
										/* @__PURE__ */ jsx("strong", { children: "评分反馈" }),
										/* @__PURE__ */ jsx("span", { children: response.feedback }),
										response.safetyCriticalError && /* @__PURE__ */ jsx("span", {
											className: "grading-critical-note",
											children: "检测到安全红线错误：本题已执行安全封顶，并阻止岗位阶段达标。"
										}),
										!!response.contradictions.length && /* @__PURE__ */ jsxs("span", { children: ["需复核的矛盾：", response.contradictions.join("；")] }),
										/* @__PURE__ */ jsxs("small", { children: [
											response.gradingMethod,
											response.semanticSimilarity !== null ? ` · 语义相似度 ${Math.round(response.semanticSimilarity * 100)}%` : "",
											response.keyPointCoverage !== null ? ` · 评分点覆盖 ${Math.round(response.keyPointCoverage * 100)}%` : ""
										] })
									]
								}),
								!!question.source_refs?.length && /* @__PURE__ */ jsxs("div", {
									className: "quiz-evidence-refs",
									children: ["依据：", question.source_refs.slice(0, 3).join("、")]
								})
							]
						}),
						displayedQuizError && /* @__PURE__ */ jsxs("div", {
							className: "quiz-inline-error",
							role: "alert",
							"aria-live": "assertive",
							children: [
								/* @__PURE__ */ jsx("strong", { children: "Quiz 操作未完成" }),
								/* @__PURE__ */ jsx("span", { children: displayedQuizError }),
								/* @__PURE__ */ jsx("small", { children: "已生成内容和作答草稿均会保留，解决后可直接重试。" })
							]
						}),
						/* @__PURE__ */ jsxs("div", {
							...
							children: [/* @__PURE__ */ jsx("button", {
								...
								children: "上一题"
							}), !isSubmitted ? /* @__PURE__ */ jsx("button", {
								type: "button",
								className: "primary-button",
								disabled: !selectedAnswer?.trim() || generationRunning || gradingQuestionId === question.id,
								onClick: () => void submitAnswer(),
								children: gradingQuestionId === question.id ? "语义评分中…" : "提交答案"
							}) : /* @__PURE__ */ jsx("button", {
								type: "button",
								className: "primary-button",
								disabled: index === activeSession.questions.length - 1,
								onClick: () => moveToQuestion(index + 1),
								children: index === activeSession.questions.length - 1 ? activeSession.status === "completed" ? "测验已完成" : "等待完成" : "下一题"
							})]
						})
					] })]
				})]
...
function QuizHistoryPanel({ sessions, onOpen }) {
	if (!sessions.length) return /* @__PURE__ */ jsxs("div", {
		className: "quiz-history-empty",
		children: [/* @__PURE__ */ jsx("span", { children: "暂无 Quiz 记录" }), /* @__PURE__ */ jsx("p", { children: "新生成的题目会在后端返回后立即保存，未完成的测验也可以继续。" })]
	});
	return /* @__PURE__ */ jsxs("div", {
		className: "quiz-history-panel",
		children: [/* @__PURE__ */ jsx("div", {
			className: "quiz-history-heading",
			children: /* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("strong", { children: "Quiz 历史" }), /* @__PURE__ */ jsx("p", { children: "保留最近 50 次测验的题目、答案、解析和知识来源。" })] })
		}), /* @__PURE__ */ jsx("div", {
			className: "quiz-history-list",
			children: sessions.map((session) => {
				const progress = quizSessionProgress(session);
				const createdAt = new Date(session.createdAt);
				const dateLabel = Number.isFinite(createdAt.getTime()) ? createdAt.toLocaleString("zh-CN", {
					month: "2-digit",
					day: "2-digit",
					hour: "2-digit",
					minute: "2-digit"
				...
				return /* @__PURE__ */ jsxs("article", {
					...
					children: [/* @__PURE__ */ jsxs("div", {
						className: "quiz-history-item-main",
						children: [
							/* @__PURE__ */ jsxs("div", {
								className: "quiz-history-title-row",
								children: [/* @__PURE__ */ jsx("strong", { children: session.topic }), /* @__PURE__ */ jsx("span", {
									className: `quiz-session-status ${session.status}`,
									children: session.status === "completed" ? "已完成" : session.status === "abandoned" ? "已暂停" : "进行中"
								})]
							}),
							/* @__PURE__ */ jsx("p", { children: session.focus || "综合知识测验" }),
							/* @__PURE__ */ jsxs("div", {
								className: "quiz-history-meta",
								children: [
									/* @__PURE__ */ jsx("span", { children: dateLabel }),
									/* @__PURE__ */ jsxs("span", { children: [
										progress.submitted,
										"/",
										progress.total,
									...
									progress.accuracy !== null && /* @__PURE__ */ jsxs("span", { children: [
										"当前得分 ",
									...
									!!session.retrieval.sourceRefs.length && /* @__PURE__ */ jsxs("span", { children: [session.retrieval.sourceRefs.length, " 个知识来源"] })
								]
							})
						]
					}), /* @__PURE__ */ jsx("button", {
						...
						children: session.status === "completed" ? "查看结果" : "继续作答"
					})]
				}, session.id);
			})
		})]
...
function QuizQuestionOverview({ session, onOpen }) {
	const groups = Object.keys(QUIZ_TYPE_LABELS).map((type) => ({
		type,
	...
	return /* @__PURE__ */ jsxs("div", {
		...
		children: [/* @__PURE__ */ jsxs("div", {
			className: "quiz-overview-heading",
			children: [/* @__PURE__ */ jsx("strong", { children: "题目概览" }), /* @__PURE__ */ jsx("span", { children: "按题型归类，点击题号跳转" })]
		}), /* @__PURE__ */ jsx("div", {
			className: "quiz-overview-groups",
			children: groups.map((group) => /* @__PURE__ */ jsxs("div", {
				className: "quiz-overview-group",
				children: [/* @__PURE__ */ jsx("span", { children: QUIZ_TYPE_LABELS[group.type] }), /* @__PURE__ */ jsx("div", { children: group.questions.map(({ question, index }) => {
					const response = session.responses[question.id];
					return /* @__PURE__ */ jsx("button", {
						type: "button",
						className: `${response?.submittedAt ? response.isCorrect ? "correct" : "wrong" : response?.selectedAnswer ? "draft" : ""} ${index === session.currentIndex ? "active" : ""}`,
						"aria-label": `跳转到第 ${index + 1} 题`,
						"aria-current": index === session.currentIndex ? "step" : void 0,
...
function LectureView({ sessions, activeSessionId, onActiveSessionChange, onSessionCreated, profile, scores, progress, capabilityEvidence, busy, runAgent, onProgressChanged }) {
	const [lectureError, setLectureError] = useState("");
	const [confirmNext, setConfirmNext] = useState(false);
	const activeLecture = sessions.find((session) => session.id === activeSessionId) ?? sessions[0] ?? null;
	const mastery = useMemo(() => activeLecture ? calculateLectureMastery(activeLecture, capabilityEvidence) : null, [activeLecture, capabilityEvidence]);
	const nextChapter = activeLecture ? nextCourseChapter(activeLecture.chapterId) : null;
	async function requestLecture(reason, chapterId, predecessor) {
		setLectureError("");
		const prompt = [
			`请为当前学习者生成“${chapterId} ${(reason === "next_stage" ? nextCourseChapter(chapterId)?.title : null) || predecessor?.chapterTitle || progress.currentChapterTitle}”阶段学习讲义。`,
		...
		let response = await runAgent(buildAgentRequest({
			userId: ACTIVE_USER_ID,
			courseId: ACTIVE_COURSE_ID,
			chapterId,
			prompt,
			contentType: reason === "next_stage" ? "next_step" : "lecture",
			scores,
			profile,
			learningProgress: progress.agentContext
		}));
		const targetChapterId = (reason === "next_stage" ? nextCourseChapter(chapterId) : null)?.id ?? chapterId;
		let session;
		try {
			...
			response = await runAgent(buildAgentRequest({
				userId: ACTIVE_USER_ID,
				courseId: ACTIVE_COURSE_ID,
				chapterId: targetChapterId,
	...
	async function generate(reason, chapterId, predecessor) {
		try {
			await requestLecture(reason, chapterId, predecessor);
		} catch (error) {
			setLectureError(error instanceof Error ? error.message : "学习讲义生成失败，请检查中央调度器");
		}
	}
	function requestNextStage() {
		...
		generate("next_stage", activeLecture.chapterId, activeLecture);
	}
	return /* @__PURE__ */ jsxs("div", {
		className: "lecture-page",
		children: [
			/* @__PURE__ */ jsxs("aside", {
				className: "lecture-library card",
				children: [/* @__PURE__ */ jsxs("div", {
					className: "lecture-library-head",
					children: [/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("h2", { children: "讲义记录" }), /* @__PURE__ */ jsx("p", { children: "已生成的讲义会自动保存" })] }), /* @__PURE__ */ jsx("span", { children: sessions.length })]
				}), /* @__PURE__ */ jsx("div", {
					className: "lecture-history-list",
					children: sessions.length ? sessions.map((lecture) => {
						const itemMastery = calculateLectureMastery(lecture, capabilityEvidence);
						return /* @__PURE__ */ jsxs("button", {
							type: "button",
							...
							children: [
								/* @__PURE__ */ jsx("span", {
									className: "lecture-history-chapter",
									children: lecture.chapterId
								}),
								/* @__PURE__ */ jsx("strong", { children: lecture.title }),
								/* @__PURE__ */ jsx("small", { children: itemMastery.status === "mastered" ? "已掌握" : itemMastery.score !== null ? `掌握度 ${itemMastery.score}%` : itemMastery.observedScore !== null ? `练习表现 ${itemMastery.observedScore}% · 证据不足` : "待评估" })
							]
						}, lecture.id);
					}) : /* @__PURE__ */ jsx("div", {
						className: "lecture-history-empty",
						children: "生成第一份讲义后，这里会形成个人讲义库。"
					})
				})]
			}),
			/* @__PURE__ */ jsx("section", {
				className: "lecture-reader card",
				children: !activeLecture ? /* @__PURE__ */ jsxs("div", {
					className: "lecture-empty",
					children: [
						/* @__PURE__ */ jsx("span", { children: "学" }),
						/* @__PURE__ */ jsx("h2", { children: "从当前阶段开始学习" }),
						...
						/* @__PURE__ */ jsx("button", {
							...
							className: "primary-button",
							disabled: busy,
							onClick: () => void generate("initial", progress.currentChapterId, null),
							children: busy ? "中央调度器生成中…" : "生成当前阶段讲义"
						}),
						lectureError && /* @__PURE__ */ jsx("p", {
							className: "inline-error",
							children: lectureError
						})
					]
				}) : /* @__PURE__ */ jsxs(Fragment$1, { children: [
					/* @__PURE__ */ jsxs("header", {
						className: "lecture-reader-head",
						children: [/* @__PURE__ */ jsxs("div", { children: [
							/* @__PURE__ */ jsxs("div", {
								className: "lecture-meta-row",
								children: [
									/* @__PURE__ */ jsxs("span", { children: ["章节 ", activeLecture.chapterId] }),
									/* @__PURE__ */ jsx("span", { children: activeLecture.chapterTitle }),
									/* @__PURE__ */ jsxs("span", { children: [activeLecture.sourceRefs.length, " 个知识来源"] })
								]
							}),
							/* @__PURE__ */ jsx("h2", { children: activeLecture.title }),
							/* @__PURE__ */ jsx("p", { children: activeLecture.summary })
						] }), /* @__PURE__ */ jsxs("div", {
							...
							children: [
								/* @__PURE__ */ jsx("button", {
									...
									className: "secondary-button",
									disabled: busy,
									onClick: () => void generate("regenerate", activeLecture.chapterId, activeLecture),
									children: "重新生成"
								}),
								/* @__PURE__ */ jsx("button", {
									type: "button",
									...
									children: nextChapter ? "生成下阶段讲义" : "已是最后阶段"
								}),
								mastery && /* @__PURE__ */ jsx("p", {
									className: mastery.recommendedForNextStage ? "ready" : "hold",
									children: mastery.message
								})
							]
						})]
					}),
					lectureError && /* @__PURE__ */ jsx("p", {
						className: "lecture-error",
						children: lectureError
					}),
					/* @__PURE__ */ jsxs("div", {
						className: "lecture-reader-body",
						children: [/* @__PURE__ */ jsx("div", {
							className: "lecture-content",
							children: activeLecture.sections.map((section, index) => /* @__PURE__ */ jsxs("article", { children: [/* @__PURE__ */ jsx("h3", { children: section.heading }), /* @__PURE__ */ jsx(MarkdownContent, {
								baseUrl: activeLecture.artifactPath ? storageMarkdownBaseUrl(activeLecture.artifactPath) : void 0,
								content: section.content
							})] }, `${section.heading}-${index}`))
						}), /* @__PURE__ */ jsxs("aside", {
							className: "lecture-mastery-panel",
							children: [
								/* @__PURE__ */ jsxs("div", {
									className: "lecture-mastery-score",
									children: [/* @__PURE__ */ jsx("span", { children: mastery?.score !== null && mastery?.score !== void 0 ? `${mastery.score}%` : "待评估" }), /* @__PURE__ */ jsx("small", { children: "当前讲义掌握度" })]
								}),
								/* @__PURE__ */ jsx("div", {
									className: "lecture-mastery-track",
									children: /* @__PURE__ */ jsx("span", { style: { width: `${mastery?.score ?? 0}%` } })
								}),
								/* @__PURE__ */ jsx("p", { children: mastery?.observedScore !== null && mastery?.observedScore !== void 0 ? `已关联作答的观察表现 ${mastery.observedScore}%，评价可信度 ${Math.round(mastery.confidence * 100)}%。` : "只统计与本讲义、章节和学习目标精确关联的新证据。" }),
								/* @__PURE__ */ jsx("div", {
									className: "lecture-requirements",
									children: mastery?.requirements.map((requirement) => /* @__PURE__ */ jsxs("div", {
										className: requirement.passed ? "passed" : "pending",
										children: [/* @__PURE__ */ jsx("span", { children: requirement.passed ? "✓" : "·" }), /* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("strong", { children: requirement.label }), /* @__PURE__ */ jsxs("small", { children: [
											requirement.current,
											" / ",
								...
								/* @__PURE__ */ jsxs("div", {
									className: "lecture-sources",
									children: [/* @__PURE__ */ jsx("strong", { children: "RAG 知识依据" }), activeLecture.sourceRefs.length ? activeLecture.sourceRefs.slice(0, 6).map((source) => /* @__PURE__ */ jsx("span", {
										title: source,
										children: source.split(/[\\/]/).pop()
									}, source)) : /* @__PURE__ */ jsx("span", { children: "本次返回未包含可展示的来源名称" })]
								})
							]
						})]
					})
			...
			confirmNext && activeLecture && nextChapter && /* @__PURE__ */ jsx("div", {
				className: "confirmation-backdrop",
				role: "presentation",
				onMouseDown: () => setConfirmNext(false),
				children: /* @__PURE__ */ jsxs("div", {
					...
					children: [
						/* @__PURE__ */ jsx("span", {
							className: "confirmation-symbol",
							children: "!"
						}),
						/* @__PURE__ */ jsx("h2", {
							id: "lecture-confirm-title",
							children: "当前知识尚未达到推荐掌握度"
						}),
						/* @__PURE__ */ jsxs("p", { children: [
							mastery?.message,
							"。如果继续，将进入章节 ",
...
function LearningProgressView({ progress, profile, scores, busy, runAgent }) {
	...
	async function generateNextPlan() {
		...
		try {
			...
			setNextPlan(asQaPayload(response)?.answer || response.final_output?.summary || "中央调度器已完成任务，但没有返回可展示的训练方案。");
			setPlanSources([...(response.rag_package?.evidence ?? []).map((item) => item.source_file ?? item.source_doc ?? ""), ...(response.rag_package?.citations ?? []).map((item) => item.source_file ?? item.source_doc ?? "")].filter(Boolean).filter((value, index, values) => values.indexOf(value) === index).slice(0, 5));
		} catch (error) {
	...
	const currentStageIndex = [
		...
		"l3",
		"job_ready"
	].indexOf(progress.currentStageId);
	return /* @__PURE__ */ jsxs("div", {
		...
		children: [
			/* @__PURE__ */ jsxs("section", {
				...
				children: [/* @__PURE__ */ jsxs("div", {
					...
					style: { "--progress": `${progress.overallProgress}%` },
					children: [/* @__PURE__ */ jsxs("span", { children: [progress.overallProgress, "%"] }), /* @__PURE__ */ jsx("small", { children: "达标条件完成度" })]
				}), /* @__PURE__ */ jsxs("div", {
					className: "progress-hero-copy",
					children: [
						/* @__PURE__ */ jsxs("span", {
							className: "eyebrow",
							children: ["当前目标 · ", progress.currentStageLabel]
						}),
						/* @__PURE__ */ jsx("h2", { children: progress.currentStageOutcome }),
						/* @__PURE__ */ jsxs("p", { children: [
							"当前章节 ",
							progress.currentChapterId,
							"「",
							progress.currentChapterTitle,
							"」。评价由客观学习证据自动计算， 用户画像只用于调整学习路径，不会直接增加能力分数。"
						] }),
						/* @__PURE__ */ jsxs("div", {
							className: "progress-summary-chips",
							children: [
								/* @__PURE__ */ jsxs("span", { children: ["已验证能力贡献 ", progress.verifiedWeight ? `${progress.weightedMastery} 分` : "待形成"] }),
								/* @__PURE__ */ jsxs("span", { children: [
									"能力暂估 ",
									progress.provisionalMastery,
									" 分"
								] }),
								/* @__PURE__ */ jsxs("span", { children: [
									"评价可信度 ",
									progress.weightedConfidence,
									"%"
								] }),
								/* @__PURE__ */ jsxs("span", { children: [
									"课程完成 ",
			...
			/* @__PURE__ */ jsxs("section", {
				...
				children: [/* @__PURE__ */ jsxs("div", {
					...
					children: [/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("h3", { children: "从入门到岗位胜任" }), /* @__PURE__ */ jsx("p", { children: "课程目录与 L1/L2/L3 岗位能力门槛共同决定进度。" })] }), /* @__PURE__ */ jsxs("span", {
						className: "model-chip",
						children: ["模型 ", progress.modelVersion]
					})]
				}), /* @__PURE__ */ jsx("div", {
					className: "career-path-steps",
					children: COURSE_PHASES.map((phase, index) => {
						...
						return /* @__PURE__ */ jsxs("article", {
							...
							children: [/* @__PURE__ */ jsx("span", {
								className: "career-step-index",
								children: completed ? "✓" : index + 1
							}), /* @__PURE__ */ jsxs("div", { children: [
								/* @__PURE__ */ jsx("strong", { children: phase.label }),
								/* @__PURE__ */ jsx("small", { children: phase.range }),
								/* @__PURE__ */ jsx("p", { children: phase.outcome })
							] })]
						}, phase.id);
					})
				})]
			...
			/* @__PURE__ */ jsxs("div", {
				...
				children: [/* @__PURE__ */ jsxs("section", {
					className: "card progress-gates-card",
					children: [/* @__PURE__ */ jsx("div", {
						className: "section-heading",
						children: /* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsxs("h3", { children: [progress.currentStageLabel, " 达标条件"] }), /* @__PURE__ */ jsx("p", { children: "安全和实操属于硬门槛，不能由其他高分抵消。" })] })
					}), /* @__PURE__ */ jsx("div", {
						className: "progress-gates",
						children: progress.gates.map((gate) => /* @__PURE__ */ jsxs("div", {
							className: `progress-gate ${gate.passed ? "passed" : "pending"}`,
							children: [/* @__PURE__ */ jsx("span", {
								className: "gate-state",
								children: gate.passed ? "✓" : "!"
							}), /* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("strong", { children: gate.label }), /* @__PURE__ */ jsxs("p", { children: [
								"当前 ",
								gate.current,
								" · 要求 ",
								gate.requirement
					...
					className: "card evidence-layers-card",
					children: [
						/* @__PURE__ */ jsx("div", {
							className: "section-heading",
							children: /* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("h3", { children: "评价证据链" }), /* @__PURE__ */ jsx("p", { children: "原始事件保留审计记录，只有有效、独立且已核验的证据参与评级。" })] })
						}),
						/* @__PURE__ */ jsxs("div", {
							className: "evidence-layer-list",
							children: [
								/* @__PURE__ */ jsxs("article", { children: [/* @__PURE__ */ jsx("span", { children: "01" }), /* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("strong", { children: "原始操作记录" }), /* @__PURE__ */ jsxs("p", { children: [progress.evidence.rawTraceCount, " 条提问、测验与 Memory 操作轨迹"] })] })] }),
								/* @__PURE__ */ jsxs("article", { children: [/* @__PURE__ */ jsx("span", { children: "02" }), /* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("strong", { children: "结构化评价证据" }), /* @__PURE__ */ jsxs("p", { children: [
									progress.evidence.effectiveEvidenceCount,
									"/",
									progress.evidence.structuredEvidenceCount,
									" 条有效证据 · ",
									progress.evidence.assessedDimensionCount,
									"/8 个维度已有观察"
								] })] })] }),
								/* @__PURE__ */ jsxs("article", { children: [/* @__PURE__ */ jsx("span", { children: "03" }), /* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("strong", { children: "派生能力与岗位结论" }), /* @__PURE__ */ jsxs("p", { children: [
									progress.evidence.ratedDimensionCount,
									"/8 个维度达到评级证据门槛 · ",
						...
						/* @__PURE__ */ jsxs("div", {
							...
							children: [
								/* @__PURE__ */ jsx("strong", { children: "证据构成" }),
								/* @__PURE__ */ jsxs("span", { children: ["Quiz ", progress.evidence.quizEvidenceCount] }),
								/* @__PURE__ */ jsxs("span", { children: ["实操 ", progress.evidence.practicalEvidenceCount] }),
								/* @__PURE__ */ jsxs("span", { children: ["外部考核 ", progress.evidence.externalAssessmentCount] }),
								/* @__PURE__ */ jsxs("span", { children: ["RAG 有依据 ", progress.evidence.groundedEvidenceCount] })
							]
						})
					]
				})]
			...
			/* @__PURE__ */ jsxs("div", {
				className: "progress-grid lower",
				children: [/* @__PURE__ */ jsxs("section", {
					className: "card dimension-readiness-card",
					children: [/* @__PURE__ */ jsx("div", {
						className: "section-heading",
						children: /* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("h3", { children: "八维岗位能力" }), /* @__PURE__ */ jsx("p", { children: "观察表现、保守能力估计和评价可信度分别展示；低样本不会被判定为熟练。" })] })
					}), /* @__PURE__ */ jsx("div", {
						className: "dimension-progress-list",
						children: progress.dimensions.map((dimension) => {
							const scoreText = dimension.ratingStatus === "unassessed" ? "待评估" : dimension.ratingStatus === "insufficient" ? `暂估 ${dimension.score ?? 0} 分` : `${dimension.score ?? 0} 分`;
							const channelText = (score, status) => status === "unassessed" ? "待评估" : status === "insufficient" ? `暂估 ${score ?? 0}` : `${score ?? 0}`;
							return /* @__PURE__ */ jsxs("div", {
								className: `dimension-progress ${dimension.ratingStatus}`,
								children: [
									/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("strong", { children: dimension.label }), /* @__PURE__ */ jsxs("span", { children: [
										scoreText,
										" · 权重 ",
										dimension.weight,
										"%"
									...
									/* @__PURE__ */ jsx("div", {
										...
										children: /* @__PURE__ */ jsx("span", { style: { width: `${dimension.score ?? 0}%` } })
									}),
									/* @__PURE__ */ jsxs("small", { children: [
										"观察表现 ",
										dimension.observedScore === null ? "—" : `${dimension.observedScore} 分`,
										" · 有效证据 ",
										dimension.effectiveEvidenceCount,
										" 条 · 可信度 ",
										dimension.confidence,
										"%"
									] }),
									/* @__PURE__ */ jsxs("div", {
										className: "dimension-channel-list",
										children: [/* @__PURE__ */ jsxs("span", { children: ["知识 ", channelText(dimension.knowledgeScore, dimension.knowledgeStatus)] }), /* @__PURE__ */ jsxs("span", { children: ["实操 ", channelText(dimension.practiceScore, dimension.practiceStatus)] })]
									})
								]
							}, dimension.id);
						})
					...
					className: "card next-training-card",
					children: [
						/* @__PURE__ */ jsx("div", {
							className: "section-heading",
							children: /* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("h3", { children: "下一步学习任务" }), /* @__PURE__ */ jsx("p", { children: "由中央调度器结合 Memory、当前门槛和 RAG 知识库生成。" })] })
						}),
						/* @__PURE__ */ jsx("div", {
							className: "blocker-list",
							children: (progress.blockers.length ? progress.blockers : ["当前阶段已达标，可进入下一阶段复核"]).slice(0, 4).map((blocker) => /* @__PURE__ */ jsxs("p", { children: [/* @__PURE__ */ jsx("span", { children: "→" }), blocker] }, blocker))
						}),
						/* @__PURE__ */ jsx("button", {
							type: "button",
							className: "primary-button",
							...
							children: busy ? "中央调度器处理中…" : "生成下一步训练方案"
						}),
						planError && /* @__PURE__ */ jsx("p", {
							className: "inline-error",
							children: planError
						}),
						nextPlan && /* @__PURE__ */ jsxs("div", {
							className: "next-plan-result",
							children: [/* @__PURE__ */ jsx(MarkdownContent, { content: nextPlan }), !!planSources.length && /* @__PURE__ */ jsxs("div", {
								className: "next-plan-sources",
								children: [/* @__PURE__ */ jsx("strong", { children: "RAG 依据" }), planSources.map((source) => /* @__PURE__ */ jsx("span", { children: source.split(/[\\/]/).pop() }, source))]
							})]
						})
					]
				})]
			...
			/* @__PURE__ */ jsx("p", {
				className: "progress-method-note",
				children: "说明：岗位能力采用固定权重、贝叶斯小样本收缩、独立尝试去重和来源审核。Quiz 只能形成知识证据；操作、质量与维护能力还必须有已复核的实操证据。本模型不等同于国家职业资格证书。"
			})
		]
	});
}
function MemoryView({ profile, assessment, progress, setProfile, events, setEvents, suggestions, setSuggestions, busy, onReload, onSaved }) {
	const capabilityResults = capabilityResultList(assessment, profile.level);
	function applySuggestion(suggestion) {
		const patch = suggestion.patch;
		if (patch.op === "remove") setProfile((current) => ({
			...current,
			[patch.path]: ""
		}));
	...
	return /* @__PURE__ */ jsx("div", {
		...
		children: /* @__PURE__ */ jsxs("div", {
			...
			children: [/* @__PURE__ */ jsxs("div", {
				className: "section-intro",
				children: [/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("h2", { children: "学习者 Memory" }), /* @__PURE__ */ jsx("p", { children: "Memory 由结构化画像、逐题评估证据和更新记录组成。能力分数只由 Quiz 或后续实训的客观结果计算，不能手动修改。" })] }), /* @__PURE__ */ jsx("button", {
					type: "button",
					className: "quiet-button",
					disabled: busy,
					onClick: () => void onReload(),
					children: busy ? "同步中…" : "从后端刷新"
				})]
			}), /* @__PURE__ */ jsxs("div", {
				className: "memory-grid",
				children: [/* @__PURE__ */ jsxs("div", {
					className: "memory-side",
					children: [/* @__PURE__ */ jsxs("section", {
						...
						children: [
							...
							/* @__PURE__ */ jsx("p", {
								className: "card-subtitle",
								children: "对应接口中的 learner_profile，修改后自动用于下一次请求。"
							}),
							/* @__PURE__ */ jsxs("div", {
							...
							/* @__PURE__ */ jsxs("div", {
								...
								children: [/* @__PURE__ */ jsx("span", {
									className: "card-subtitle",
									style: { margin: 0 },
									children: "本地自动缓存，点击保存后同步到后端"
								}), /* @__PURE__ */ jsx("button", {
									type: "button",
									className: "primary-button",
									disabled: busy,
									onClick: () => void onSaved(),
									children: busy ? "保存中…" : "保存到后端"
								})]
							})
						]
					}), /* @__PURE__ */ jsxs("section", {
						className: "card scores-card",
						children: [
							/* @__PURE__ */ jsxs("div", {
								...
								children: [/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("h3", {
									...
									className: "card-subtitle",
									children: "观察正确率、保守能力估计、置信度和岗位评级分开展示。"
								})] }), /* @__PURE__ */ jsx("span", {
									className: "assessment-version",
									children: assessment.modelVersion
								})]
							}),
							/* @__PURE__ */ jsxs("div", {
								className: "assessment-summary",
								children: [
									/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("strong", { children: assessment.effectiveEvidenceCount }), /* @__PURE__ */ jsx("span", { children: "独立有效证据" })] }),
									/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsxs("strong", { children: [assessment.ratedDimensionCount, "/8"] }), /* @__PURE__ */ jsx("span", { children: "可评级维度" })] }),
									/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("strong", { children: assessment.ratedDimensionCount ? `${progress.weightedMastery}%` : "待形成" }), /* @__PURE__ */ jsx("span", { children: "已验证岗位能力" })] })
								]
							}),
							/* @__PURE__ */ jsx("p", {
								className: "card-subtitle",
								children: "一两道题即使全对，也只显示观察表现和“证据不足”，不会评为熟练。 Quiz 只形成知识证据，操作能力必须等已审核的实操证据。"
							}),
							/* @__PURE__ */ jsx("div", {
								className: "capability-list",
								children: capabilityResults.map((result) => /* @__PURE__ */ jsxs("article", {
									className: "capability-row",
									children: [
										/* @__PURE__ */ jsxs("div", {
											className: "capability-row-head",
											children: [/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("strong", { children: result.label }), /* @__PURE__ */ jsx("span", { children: result.description })] }), /* @__PURE__ */ jsxs("div", {
												className: "capability-score",
												children: [/* @__PURE__ */ jsx("strong", { children: result.score === null ? "—" : result.ratingStatus === "rated" ? result.score : `~${result.score}` }), /* @__PURE__ */ jsx("span", { children: result.masteryLabel })]
											})]
										}),
										/* @__PURE__ */ jsx("div", {
										...
										/* @__PURE__ */ jsxs("div", {
											...
											children: [
												/* @__PURE__ */ jsxs("span", { children: ["观察表现：", result.observedScore === null ? "待评估" : `${result.observedScore}%`] }),
												/* @__PURE__ */ jsxs("span", { children: [result.effectiveEvidenceCount, " 条独立证据"] }),
												/* @__PURE__ */ jsxs("span", { children: [result.independentAttemptCount, " 次独立测验"] }),
												/* @__PURE__ */ jsxs("span", { children: ["置信度：", result.confidenceLabel] }),
												/* @__PURE__ */ jsx("span", { children: result.sourceCount ? `${result.sourceCount} 个知识来源` : "尚无 RAG 来源" })
											]
										})
									]
								}, result.id))
							...
							/* @__PURE__ */ jsxs("details", {
								...
								children: [/* @__PURE__ */ jsx("summary", { children: "查看评分方法" }), /* @__PURE__ */ jsxs("ol", { children: [
									/* @__PURE__ */ jsx("li", { children: "观察表现是已作答题目的难度、时间和评分可信度加权得分率。" }),
									/* @__PURE__ */ jsx("li", { children: "能力估计加入中性先验，避免一题全对直接变成 100 分。" }),
									/* @__PURE__ */ jsx("li", { children: "同一题重复作答只保留首次独立证据；同次测验的同一知识点最多计一条。" }),
									/* @__PURE__ */ jsx("li", { children: "至少 4 条独立证据、3 个知识点和 2 次测验，才能形成能力等级。" }),
									/* @__PURE__ */ jsx("li", { children: "理论 Quiz 和已审核实操分开计分；低置信 AI 主观评分先进入待复核。" })
								] })]
							})
						]
					})]
					...
					className: "memory-side",
					children: [/* @__PURE__ */ jsxs("section", {
						className: "card memory-log-card",
						children: [
							/* @__PURE__ */ jsx("h3", {
								className: "card-title",
								children: "待确认的画像建议"
							...
							/* @__PURE__ */ jsx("p", {
								className: "card-subtitle",
								children: "来自 profile_update_suggestions，不会自动覆盖现有画像。"
							}),
							suggestions.length ? suggestions.slice(0, 4).map((suggestion) => /* @__PURE__ */ jsxs("div", {
								className: "suggestion-card",
								children: [
									/* @__PURE__ */ jsxs("strong", { children: ["画像字段 · ", suggestion.patch.path] }),
									/* @__PURE__ */ jsx("p", { children: suggestion.patch.reason }),
									/* @__PURE__ */ jsxs("div", {
							...
							}, suggestion.id)) : /* @__PURE__ */ jsx("div", {
								className: "empty-activity",
								children: "完成一次问答后，这里会显示 Agent 返回的画像更新建议；能力分数不接受主观建议。"
							})
						]
					}), /* @__PURE__ */ jsxs("section", {
						className: "card memory-log-card",
						children: [
							/* @__PURE__ */ jsx("h3", {
								className: "card-title",
								children: "Memory 更新记录"
							}),
							/* @__PURE__ */ jsx("p", {
								className: "card-subtitle",
								children: "记录已确认的学习行为和画像变更。"
							}),
							/* @__PURE__ */ jsx("div", {
								className: "memory-log",
								children: events.slice(0, 6).map((event) => /* @__PURE__ */ jsxs("article", {
									className: "memory-event",
									children: [/* @__PURE__ */ jsxs("div", {
										className: "memory-event-head",
										children: [/* @__PURE__ */ jsx("strong", { children: event.title }), /* @__PURE__ */ jsx("time", { children: event.time })]
									}), /* @__PURE__ */ jsx("p", { children: event.detail })]
								}, event.id))
							})
						]
					})]
...
//#endregion
export { LearningWorkspace as default };
